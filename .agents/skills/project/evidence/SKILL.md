...
`noAggregateEvidence` needs no completion treatment. The corpus offers selected units and never an aggregate scope, so it is already confined for every reference; `role` earns a trigger only because `@evidence ` cannot match a line that opens a parenthesis.
