...
**A unit identity carries its symbol kind, so one member address can be two units or two members one unit, and both directions are open boundaries rather than the rule.** Splitting: `interface I { charge: () => void }` beside `interface I { charge: Handler }` is one member that materializes a function unit and a property unit under one address, and a citation of it is reported ambiguous. TypeScript accepts the source because the two annotations denote one type, and these rules read no checker, so nothing here can see that they do. Folding: `interface ISale { settle: () => void }` beside `namespace ISale { export const settle = (): void => {} }` is two members, in two spaces, that materialize one unit, because the kinds coincide and the identity is the whole key. That one is the worse of the two, because it leaves the coverage denominator with no diagnostic at all, while the split is loud and names its repair.

Both are reachable in source TypeScript accepts, and neither is closed here. The reason is not that a repair is unavailable: the doubled member arrives twice through the member collector while the namespace member arrives through the namespace one, so a fold keyed on a container's own repeated member is confined by construction. It is that closing the split alone leaves the silent fold standing and makes the model less coherent rather than more, and that a fold has to decide which kind wins where nothing observed the type. Decide the address-collision question once, for both directions, rather than patching the face that reported itself.
