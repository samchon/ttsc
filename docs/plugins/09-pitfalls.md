...
If you really need cache invalidation on a non-source file, set `TTSC_CACHE_DIR` to a fresh dir for the run, or delete `node_modules/.ttsc/` (falling back to `.ttsc/` in projects without `node_modules/`).
