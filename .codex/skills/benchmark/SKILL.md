...
- **Graph arms**: `graph.mjs --arm=baseline --tools=baseline` records only the empty-MCP baseline. Later `--arm=graph --tools=ttsc-graph,codegraph` runs add comparator samples against those website baseline cells. Prompt families are `dedicated` and `common`; `project-specific` and `shared-onboarding` remain accepted aliases. A graph-arm sample that completes with zero MCP calls, or that falls back to shell source reads/searches, is invalid for graph measurement and must be retried or left failed, never treated as a comparator median. If an arm has samples but no valid sample, `graph.mjs` leaves the report/audit on disk and exits failed instead of publishing that cell.
- **Graph sampling**: publication cells use N=5 and report medians. N=1 is for smoke tests and repeated optimization loops only; do not publish N=1 as a benchmark result.
