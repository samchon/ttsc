...
- **Fixtures live outside the repo**: the graph runner clones into `../ttsc-benchmark-work/`, never under `experimental/benchmark/`. The measured agent's cwd is the fixture clone, and Claude Code and Codex both walk the parent chain for `CLAUDE.md` / `AGENTS.md`; a fixture inside this repo hands ttsc's own agent instructions to every cell (a vscode run was caught reading this repo's `AGENTS.md` instead of touring vscode). A graph number measured from a fixture under the repo is contaminated and must be re-measured, not published.
- **Agent-visible names matter**: the fixture folder is what the agent sees as its cwd, so it carries no `ttsc-benchmark-` prefix (`vue@ttsc`, not `ttsc-benchmark-vue@ttsc`) — the prefix makes an agent hunt for harness code instead of touring the source. `performance.mjs` names the folder from `workName` and the GitHub repo from `repoName`; typeorm keeps the prefixed folder because pnpm wrote absolute symlinks into it.
