...
/**
 * Read-only renderer for messages captured from a sandboxed `console.*` call.
 *
 * Modelled on typia's playground ConsoleViewer (see
 * `typia/website/src/components/playground/ConsoleViewer.tsx`) but trimmed
 * down to tailwind classes so it matches the ttsc dark theme directly.
 *
 * `value` is the captured argv (the spread of `console.log(...args)`), so
 * `console.log("user:", user)` shows up as a single row with both pieces
 * rendered inline, separated by a space — same as a real DevTools console.
 */
  ...
  return (
    <div className="h-full w-full overflow-auto p-3 font-mono text-[12px] leading-snug">
      {messages.map((msg, i) => (
        ...
        >
          <span
            className={`shrink-0 text-[10px] uppercase tracking-wider w-12 ${typeColor(msg.type)}`}
          >
            {msg.type}
          </span>
    ...
    return (
      <span>
        ...
        {entries.map(([k, v], idx) => (
          <span key={k}>
            {idx > 0 ? ", " : " "}
            <span className="text-blue-300">{k}</span>: {formatValue(v, depth + 1)}
          </span>
        ))}
        {" }"}
      </span>
