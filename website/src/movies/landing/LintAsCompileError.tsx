...
const DIAGNOSTIC = [
  { type: "error", code: "TS2322", path: "src/index.ts:3:7", message: "Type 'number' is not assignable to type 'string'." },
  { type: "error", code: "TS17397", path: "src/index.ts:2:5", message: "[prefer-const] Use const instead of let." },
  { type: "error", code: "TS11966", path: "src/index.ts:1:1", message: "[no-var] Unexpected var, use let or const instead." },
];
...
{SOURCE_LINES.map((s, i) => (
  <div key={i} className="text-neutral-400">
    <span className="text-neutral-600 select-none">{s.line ? `${s.line}  ` : "   "}</span>
    {s.text}
    {s.underline && (
      <>
        {"\n"}
        ...
        <span className="text-red-400">{s.underline}</span>
              ...
              {"\n"}
              <span className="text-neutral-500">Found 3 errors in the same file.</span>
            </pre>
          </div>
        </FadeIn>
      </div>
