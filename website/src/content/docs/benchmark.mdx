...
| ---------- | ------------------ | ------------- | ----- |
| Tokens     | 1,877,370          | 396,283       | 79%   |
| Tool calls | 51.5               | 4.5           | 91%   |
| Cost       | $0.305             | $0.149        | 51%   |
| Wall time  | 159s               | 56s           | 65%   |

The agent read zero files in all ten graph runs. It answered from `graph_explore` alone. The baseline read 18 to 29 files per run to reconstruct the same flow.

The mechanism is simple. The graph hands the agent the relevant declarations, their checker-resolved relationships, and their source in a few calls, so it stops fanning out across files.

#### The win holds across models

The same Excalidraw A/B on two more models. Every figure is the median of the runs published to `graph.json`: ten per cell for the Claude models, two for GPT-5.5 via codex, so read the GPT-5.5 row as a direction, not a settled number.

| Model (Excalidraw) | Tokens saved | Tool calls saved | Graph behavior |
| --- | --- | --- | --- |
| Claude Sonnet 4.6 | 79% | 91% | answers from the graph, reads 0 files |
| Claude Opus 4.8 | 68% | 86% | answers from the graph, reads 0 files |
| GPT-5.5 (via codex) | 25% | 8% | rarely calls the graph; explores with the shell |

Both Claude models read zero files in all ten graph runs and answer from the graph. Opus is the most thorough reader, and on an earlier graph it opened source throughout; on the current graph it stops at `graph_explore`.

The outlier is the codex harness driving GPT-5.5, which leans on the shell and calls the graph rarely. Where a model reaches for the graph it trusts the resolved relationships enough to stop reading files; where it does not, the graph cannot help it.
