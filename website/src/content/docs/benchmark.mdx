...
GPT-5.5 through the codex harness is the exception, and an honest one. In half the graph runs it never calls `graph_explore` and explores entirely with the shell, so the median saving is zero. The server advertises the same tool and the same instructions to every client; whether the agent reaches for it is the harness's decision, not the graph's. This is the same lesson the cross-repository numbers teach — agent cost swings with the model and harness, not only the tool.
...
The VS Code A/B is not in the published `graph.json` yet. Run it with the daemon (see [Reproduce](#reproduce-1)) to add it.
