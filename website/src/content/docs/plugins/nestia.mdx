...
The full pipeline runs everywhere `ttsc` owns compilation, `ttsc --emit`, `ttsx`, and every [`@ttsc/unplugin`](/docs/ttsc/bundler) bundler adapter. One install; the transforms run wherever NestJS code is compiled. VS Code uses the same project config for TypeScript-Go and supported plugin diagnostics/actions, but editor integration does not rewrite emit.
