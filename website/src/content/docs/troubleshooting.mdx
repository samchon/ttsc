...
You need the [VSCode extension](/docs/setup#editor-vscode). The vanilla TypeScript language service doesn't know about plugins; the extension is what surfaces plugin diagnostics in the editor stream. **Not yet on Marketplace** — build `packages/vscode-ttsc` from the workspace and `code --install-extension` the produced `.vsix`.
