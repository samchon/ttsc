...
A tag written in a comment the parser attaches to no declaration is reported rather than discarded. A `//` line comment is not documentation to TypeScript, and neither is a block between the braces of a destructuring pattern, because a binding element is given none. Both are one edit away from a block that works, and both used to do nothing without saying so. A tag behind a third or fourth slash is answered the same way, while a tag inside documentation an author commented out is left alone, because there the repair is to delete the block or restore the code rather than to move the tag. Text that merely looks like a tag inside a string or template literal is not a comment and is not reported.
...
A tag written as prose rather than inside a comment is reported rather than discarded, because the tag renders invisibly either way and an author cannot see which one they wrote. An example is not: a tag inside a fenced or indented code block, or mentioned mid-sentence, stays silent.
