...
The parenthesized relation is optional, and only `@evidence` takes one. An exclusion says the claim does not cover the target rather than how it does, so it has no relation to name. Write the relation immediately after the tag name, as one word with no space and no parenthesis inside. The space is what keeps `@evidence (target)` meaning what it always meant, so a citation written before relations existed is untouched.

Anything else in that position is left where the author wrote it and becomes the target. `@evidence(produces here)`, `@evidence(produces)docs/spec.md#x` with the space missing, and `@evidenceExclude(produces)` all stay declarations and all report: as an unresolved target when a reason follows, and as a malformed declaration when the parenthesis was the whole body. A dropped acknowledgement is the one outcome the graph exists to prevent, so nothing here fails silently.
...
Every row above that covers something below its target does so because the reference asks for it by default. A reference setting [`noAggregateEvidence`](/docs/evidence/claims#reference-policies) drops that for `@evidence`, so a citation there answers for the unit it names and nothing beneath it, or for nothing at all when the target it names is not itself a selected unit.
...
`uniqueEvidence` and `singleEvidencePerSymbol` on a reference tighten the first two rules where an obligation needs an owner, and `noAggregateEvidence` turns the second one off for `@evidence`, so a parent citation stops accepting responsibility for its subtree while a parent exclusion still decides the whole of it. `role` tightens a different axis: the parenthesized word is the relation this host claims, and a reference declaring `role` is discharged by positive evidence naming that relation and by no other positive evidence. An `@evidenceExclude` still answers it, because a reference refuses exclusions through `noEvidenceExclude` and through nothing else. [Reference policies](/docs/evidence/claims#reference-policies) covers all five.
...
It follows the hierarchy `@evidence` follows by default, and keeps following it under `noAggregateEvidence`, which confines positive evidence alone, so excluding an H2 also excludes its selected H3 and H4 descendants, and excluding a type or namespace excludes its selected children.
