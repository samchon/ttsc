...
Anything else in that position is left where the author wrote it and becomes the target, which reports as unresolved: `@evidence(produces here)`, `@evidence(produces)docs/spec.md#x` with the space missing, and `@evidenceExclude(produces)` all name a target rather than quietly ceasing to be declarations. A dropped acknowledgement is the one outcome the graph exists to prevent, so nothing here fails silently.
