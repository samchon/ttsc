...
Every "and its selected descendants" above is what the reference asks for by default. A reference setting [`noAggregateEvidence`](/docs/evidence/claims#reference-policies) drops it for `@evidence`, so a citation there answers for the unit it names and nothing below it.
