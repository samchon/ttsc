...
`uniqueEvidence` and `singleEvidencePerSymbol` on a reference tighten the first two rules where an obligation needs an owner. `role` tightens a different axis: written as `@evidence(produces) <target> <reason>`, the parenthesized word is the relation this host claims, and a reference declaring `role` is discharged only by that relation. [Reference policies](/docs/evidence/claims#reference-policies) covers all three.
