...
Paths resolve relative to the file containing the citation. Use `/` or `\` as file separators, `%20` for a space, `%23` for a literal `#`, and `%25` for a literal percent sign. Path identity remains case-sensitive. The accessor is separate: its slashes and backslashes are literal name characters.
