...
`uniqueEvidence` and `singleEvidencePerSymbol` on a reference tighten the first two rules where an obligation needs an owner, and `checklist` on a Markdown reference reverses the second one: a parent target stops accepting responsibility for the subtree, because a checklist item is answered by name or not at all. [Reference policies](/docs/evidence/claims#reference-policies) covers all three.
