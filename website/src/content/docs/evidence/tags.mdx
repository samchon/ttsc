...
Every row above that covers something below its target does so because the reference asks for it by default. A reference setting [`noAggregateEvidence`](/docs/evidence/claims#reference-policies) drops that for `@evidence`, so a citation there answers for the unit it names and nothing beneath it.
