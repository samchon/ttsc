...
| `{@link sales.IShoppingSale.price}` | One member of an exported interface or object-shaped type alias, whether it carries data or is written as a callable |
| `{@link sales.ShoppingSale.prototype.price}` | One instance member of an exported class |
...
A class member takes the path a consumer would actually write to reach it, so an instance member goes through `prototype` and a static member does not. A constructor parameter property is an instance member and is addressed the same way as the field it declares. [Claims and References](/docs/evidence/claims#typescript-kinds) covers which members are selected.
