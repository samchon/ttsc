...
- `"property"` selects direct properties of exported type-level symbols, public non-callable fields of exported classes, plus exported `const`, `let`, and `var` at module or namespace scope. A module-scope variable that is not function-valued is a property, including a function-typed declaration; a class field is the one place that reads the other way.
...
A field that holds a function is a `function` rather than a `property`, and it takes either spelling: `handler = () => {}` and `declare charge: () => void` are both callables of the class. This is the opposite of the module-scope rule above, where only the initializer decides and a `const parse: () => void` stays a property. The two differ because a class field's declared type is its contract, while a module-scope variable's is a description of a value that already exists.
