...
A diagnostic that asks you to correct `root` quotes the spelling you declared, so an absolute root is found by searching `lint.config.ts` for the exact text in the message. File locations stay project-relative in the same message, because one names the property to edit and the other names the path to open.
