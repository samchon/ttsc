...
| `requireReview` | markdown, typescript | Every acknowledgement owes an unexpired `@evidenceReview`. |
...
- **`requireReview`** makes every acknowledgement here owe an [`@evidenceReview`](/docs/evidence/tags) naming the same target, carrying a fingerprint of the cited scope's current content. When that content changes the fingerprint stops matching and the build fails again with the new value in the diagnostic. Expiry is the point: without it a review is written once and stays green forever, and on a second pass over a large citation set nothing tells you which reviews were written against content that has since moved.

What `requireReview` proves is narrow, and overstating it is worse than not having it. It proves that a separately addressed statement exists and was written against the cited content as it now stands. It does not prove anyone read that content, since the diagnostic states the expected fingerprint, and nothing here judges whether prose is sincere.

The fingerprint covers the cited unit and its structural subtree, so it belongs to the address a citation names rather than to the reference that asked for it. One tag carries one fingerprint however many references it acknowledges.

Markdown and TypeScript references accept it. A Swagger or Prisma reference is refused at decode: those loaders report unit identities rather than unit content, so no per-unit fingerprint exists to compare against, and the whole-document digest they do report would expire every review in a document whenever any part of it changed.
