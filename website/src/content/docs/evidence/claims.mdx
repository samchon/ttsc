...
| `evidenceExcludeCarriers` | Files allowed to host this claim's `@evidenceExclude`. Refused beside a `checklist` reference. |
...
| `checklist` | markdown | Every selected claim host must acknowledge every unit here. |
...
`root` may name a symbolic link or a Windows junction to a directory, or sit inside one, which is what a package manager installs for a linked workspace dependency. The population is read through it and the link changes nothing else. A citation target stays relative to the root and names nothing above it, a file location is still the path you open to reach the file, and the directory the link points at appears in neither. A linked directory inside the population is not descended into, while a linked file inside it is read like any other file.

A diagnostic that asks you to correct `root` quotes the spelling you declared, normalized to forward slashes and with `.` and redundant segments removed, rather than a path it derived. A file location in the same message is derived instead: it is spelled the way you would open the file, which is project-relative wherever a relative spelling exists and absolute where none does, which on Windows means a root on another volume or a UNC share. One names the property to edit and the other names the path to read.
...
- **`singleEvidencePerSymbol`** requires exactly one distinct unit from every host the claim's `symbol` selector picks. A host with no `@evidence` counts as zero and fails, as does a host citing two units. One test proving one operation stays reviewable; the same function citing eight operations proves only that eight names appear in its JSDoc. A reference whose population came back empty is reported as empty and judges no host, because there is no unit any host could have cited.
...
Counting is by identity, not by text. Repeated tags for one unit count once, merged declarations and overload sets remain one host, and an aggregate target contributes every selected descendant in its scope, so citing a parent of two selected units counts as two. A `checklist` reference is the one place that last clause does not hold.
...
A population that failed to load establishes no cardinality at all: the loader failure is preserved rather than turned into a count. A healthy population that is merely empty is a complete denominator, and the count of zero against it is true, but no per-host policy reports that count. The empty population is already an error naming itself, and a finding on each host beneath it would ask every one of them to cite a unit that does not exist.

### Checklist documents

The four properties above tighten a count inside the per-reference obligation. `checklist` does something else: it gives the obligation a host dimension, so every selected claim host answers every unit rather than the population being discharged once by whichever host got there first.
...
Set it where a document is read down a column rather than consulted: development principles, review rules, a release gate. The denominator is the claim's complete selected host population, so a host carrying no tag owes every item instead of being absent from the count. Either tag answers an item, and `@evidenceExclude` is how a host records that an item does not apply to it. Pair the option with `noEvidenceExclude` to refuse that answer and demand positive evidence from every host.

A positive citation answers the item it names and nothing beneath it. Citing a heading that is an item and also contains items stays legal and answers that one heading, while a target naming no item at all is reported as an aggregate; the items it covered are not listed again on that host, because that diagnostic already names them. Under the default selector the document itself is an item, so without both halves one `@evidence docs/rules.md` would cascade through every heading and tick every box. `@evidenceExclude` keeps the cascade, because "none of this applies here" is one reviewed decision however many items it covers.

Every acknowledgement is one host's answer, so a tag standing on no selected host of this claim answers nothing here, and is reported where it sits once nothing else consumes it. Carrier eligibility is wider than the host gate, so the same tag may be an ordinary sibling reference's gathered exclusion or an overlapping claim's own answer, and only a tag that discharges no obligation anywhere is refused. The report covers a declaration whose kind the claim does not select and one whose position materialized no unit at all. `evidenceExcludeCarriers` is refused beside this option for the same reason: gathering exclusions into another file leaves every host outside it unable to record that an item does not apply. Duplicate and conflict detection moves to the host as well: two hosts excluding one item, and one host citing an item another host excludes, are the expected state of a checklist rather than a duplicate and a contradiction.

`uniqueEvidence` and `singleEvidencePerSymbol` are refused beside it at configuration time. A checklist requires every host to cite every unit, which the first forbids the moment a claim has two hosts and the second forbids the moment the population has two units, and a graph with one host and one unit would satisfy all three by accident until the second file arrived.

Pair it with `requireReview` for the property a checklist is usually wanted for. Each host's acknowledgement then carries the fingerprint of that item alone, so editing one item expires every host's answer to it and nothing else.

Only a Markdown reference accepts it. Per-host coverage is not a statement the other populations support: it would ask every test to exercise every Swagger operation, or every file to cite every Prisma model.
...
A `checklist` reference is refused beside these globs, because it makes every acknowledgement one host's own answer and confining exclusions to other files would leave every host outside them unable to record that an item does not apply. A checklist reference that also declares `noEvidenceExclude` accepts no exclusion to gather, so that combination is allowed.
...
→ [Evidence Tags](/docs/evidence/tags) for the grammar that satisfies these obligations, or [Examples](/docs/evidence/examples) for complete graphs built from these pieces.
