...
| `role` | all | Only a declaration naming this relation discharges the reference. |
...
- **`role`** is the only one that asks what an acknowledgement is rather than how many there are. Without it every obligation is a reachability obligation, some host citing some unit, which cannot say that a unit must be produced rather than mentioned, that a read is not discharged by a write, or that a test must prove an operation works rather than that it refuses. Declare it and only `@evidence(<role>) <target> <reason>` naming the same word discharges the reference; another relation, or none, leaves the unit uncovered and the diagnostic names the relation it wanted. The vocabulary is yours, and the rule checks that the relation asked for is the relation claimed rather than that the claim is true.
