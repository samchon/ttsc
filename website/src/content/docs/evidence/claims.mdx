...
`root` may name a symbolic link or a Windows junction to a directory, which is what a package manager installs for a linked workspace dependency. The population is read through it and nothing about the link reaches the addresses. A citation target is relative to the root and names no directory at all, a file location is spelled the way you would open the file, and the directory the link points at appears in neither. A linked directory inside the population is not descended into, while a linked file inside it is read like any other file.

A diagnostic that asks you to correct `root` quotes the spelling you declared, normalized to forward slashes and with `.` and redundant segments removed, rather than a path it derived. A file location in the same message is derived instead: it is spelled the way you would open the file, which is project-relative wherever a relative spelling exists and absolute for a root on another Windows volume, where none does. One names the property to edit and the other names the path to read.
