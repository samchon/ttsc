...
A positive citation answers the item it names and nothing beneath it, and a target naming no item at all is reported as an aggregate. That report is the complete statement of what to repair: the items the refused target covered are not listed again on that host's shortfall, because the aggregate diagnostic already names them and names the one repair. Both halves are needed. Citing a heading that is an item and also contains items stays legal, and it answers that one heading; the headings under it are still owed. Under the default selector the document itself is an item, so refusing only the unnamed scope would leave one `@evidence docs/rules.md` cascading through every heading and ticking every box. `@evidenceExclude` keeps the cascade, because "none of this applies here" is one reviewed decision however many items it covers, and a host that does not participate at all should not owe one tag per item to say so.
