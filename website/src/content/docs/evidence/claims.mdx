...
| `role` | all | Only positive evidence naming this relation discharges the reference. |
| `noAggregateEvidence` | all | An `@evidence` citation covers the unit it names and none of that unit's descendants. |
...
A citation covers its target and every selected descendant. So `symbol: "property"` on a reference can still be satisfied by one `@evidence {@link IShoppingSale} ...`, and one `@evidence prisma:Sale ...` discharges every selected column and relation beneath that model. A reference can turn that cascade off for `@evidence` with [`noAggregateEvidence`](#reference-policies), which is what a population whose citing hosts do not own whole subtrees wants. An `@evidenceExclude` still covers the descendants of its target there, because one reviewed decision per subtree is what an exclusion is.
...
Five opt-in properties tighten a single reference:
...
- **`noAggregateEvidence`** answers each selected unit by its own name. Ordinarily a citation of a containing scope answers for every selected unit inside it, which is right where the citing host owns the whole subtree and wrong everywhere else: one tag naming a document's top heading discharges every requirement in it, including the ones nobody implemented, and the population reports nothing because a satisfied obligation and an obligation that does not exist look identical. Set it and a citation of a scope containing the units acknowledges none of them and is reported where it is written, while a citation of a unit that is itself selected still covers that unit and stops covering its descendants. Like `role` it constrains positive evidence only: one reviewed decision per subtree is what an exclusion is, and `noEvidenceExclude` is what refuses one.
- **`role`** is the only one that asks what an acknowledgement is rather than how many there are. Without it every obligation is a reachability obligation, some host citing some unit, which cannot say that a unit must be produced rather than mentioned, that a read is not discharged by a write, or that a test must prove an operation works rather than that it refuses. Declare it and only `@evidence(<role>) <target> <reason>` naming the same word discharges the reference; another relation, or none, leaves the unit uncovered and the diagnostic names the relation it wanted. It constrains positive evidence only: an `@evidenceExclude` names no relation and still answers, and `noEvidenceExclude` is what refuses one, so a reference that wants positive evidence under a named relation declares both. The vocabulary is yours, and the rule checks that the relation asked for is the relation claimed rather than that the claim is true.

Counting is by identity, not by text. Repeated tags for one unit count once, merged declarations and overload sets remain one host, and an aggregate target contributes every selected descendant in its scope, so citing a parent of two selected units counts as two. Under `noAggregateEvidence` that same citation counts as one when the parent is itself selected, and as zero when it is not, which is why the count says what it counted.
