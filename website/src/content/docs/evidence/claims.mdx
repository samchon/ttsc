...
Markdown and TypeScript claims can cite TypeScript evidence with `@link file.ts#Accessor <reason>`. The path is relative to the citing file. TypeScript claims can also use the existing import-scoped `@evidence {@link Symbol} <reason>` form. Prisma claims cannot cite TypeScript evidence.
...
**TypeScript** takes `files`, which only a `package` reference may omit, plus an optional `symbol`. Without `root` or `package`, entry modules come from the `ttsc` Program. Declaring `root` explicitly loads entry modules from disk; `files` then resolves against that directory. `root` and `package` cannot be combined.
...
Use a rooted reference to review an implementation in another workspace without adding evidence tags or installing the plugin there:
...
In `docs/review.md`, cite `@link ../../api/src/example.ts#Target.property <reason>`. `root` selects a code population; it does not change the citation's relative-path base. Use `root: "."` to read files outside the Program inside the current project. A root may itself be a directory link; its resolved directory is the traversal boundary. Nested links cannot admit implementations outside it. Re-exports must exist and stay within that boundary. Choose a root containing the complete exported implementation. The loader does not search other directories for a matching filename or symbol. Active Program snapshots take precedence over the saved content of the same file.
...
The rule declares its Markdown globs, Prisma globs, local Swagger paths, and rooted TypeScript directories to the `ttsc` host. Editing those sources starts the next `--watch` cycle and refreshes editor project diagnostics. A rooted code reference watches its directory because re-exports can reach files outside the entry globs. Missing files remain observable so creating or restoring one revalidates the graph.
...
File-qualified and import-scoped references already distinguish declarations in different modules. This rule independently enforces one public identity per file and its filename convention.
