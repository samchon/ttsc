...
`root` may name a symbolic link or a Windows junction to a directory, which is what a package manager installs for a linked workspace dependency. The population is read through it, and every citation target and file location is exactly what it would be if `root` named the directory directly: the link never appears in either, and neither does the path it points at. A linked directory inside the population is not descended into, while a linked file inside it is read like any other file.
