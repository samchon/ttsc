...
A type-only export list in the declaring module withholds the merge's members entirely, `export type { Sale }` and `export { type Sale }` alike, exactly as it does for the class alone, because every member of either is reached through the class value the alias does not expose. There is then no address to move a citation to, and the repair is to widen the export or drop the citation. A re-export withholds the same thing, so `export type { Sale } from "./sale.js"` publishes the class name and no member, and so do `export { type Sale } from`, `export type * from`, and `export type * as api from`. What survives a type-only edge is type-space: the name itself, and every member of an interface no class merges with or of an object-shaped type alias. What survives is type-space: the name itself, every member of an interface no class merges with or of an object-shaped type alias, and every type nested in a namespace. The withholding is a losing direction and mostly a quiet one: a `symbol: "property"` reference stops counting the member, so a build that was red for leaving it uncited goes green with no message. Which paths are loud depends on where the export is written, because a claim reads one file at a time while a reference follows the export graph. Written in the declaring module, the member leaves that file's own population, so `evidence/documented` stops asking for its block and a tag on it is refused as an unsupported host or carrier. Written as a re-export, only the reference's count changes: the declaring file still asks for the block and still accepts the tag, so an `@evidenceExclude` on such a member now excludes nothing and says nothing. One path is loud in both: a `requireReview` fingerprint of a scope reached through a type-only edge moves, because the scope no longer spans the withheld members, so every review of one expires and has to be rewritten with the new value.
