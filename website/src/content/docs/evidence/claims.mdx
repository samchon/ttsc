...
- `"property"` selects direct properties of exported type-level symbols, public class fields that are not written as callables, plus exported `const`, `let`, and `var` at module or namespace scope. A module-scope variable that is not function-valued is a property, including a function-typed declaration; a class field is the one place that reads the other way.
...
A field **written as** a callable is a `function` rather than a `property`, and it takes either spelling: `handler = () => {}` and `declare charge: () => void` are both callables of the class. This is the opposite of the module-scope rule above, where only an initializer decides and an annotation never does, so `declare const parse: () => void` stays a `property` while the identical annotation on a class field is a `function`. The two differ because a class field's declared type is its contract, while a module-scope variable's is a description of a value that already exists.
...
An interface reads differently again, and the difference is worth knowing before you select `symbol: "function"` over one. Only its property signatures materialize, so `charge: () => void` on an interface is a `property`, and a method signature written `run(): void` is no unit at all.
