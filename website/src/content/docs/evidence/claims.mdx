...
An interface merged with a same-named class declares that class's instance members, so those take the class address rather than the interface one: `interface Sale { rate: number }` beside `class Sale` is `Sale.prototype.rate`, the same address the field written in the class body would take, and a member both halves declare is one unit. A namespace is the other partner and adds statics, so it keeps `Sale.rate`. A type-only export of the merge exposes the name and no member, exactly as it does for the class alone, because every member of it is reached through the class value. **Check a citation of a merged interface's member after upgrading.** It used to be addressed from the bare name, so `{@link Sale.rate}` now reports an unreachable target and wants `{@link Sale.prototype.rate}` instead.

A local `export type { Sale }` of the merge withholds its members entirely, so there is no address to move a citation to and the repair is to widen the export or drop the citation. A barrel's `export type { Sale } from "./sale.js"` is not that form and still publishes them, which is the same gap the class half has. The withholding is a losing direction and mostly a quiet one: a `symbol: "property"` reference stops counting the member, so a build that was red for leaving it uncited goes green with no message, and `evidence/documented` stops asking for its block. The loud paths are a tag written on a withheld member, refused as an unsupported host or carrier, and a reference left with no unit at all. The class half of a type-only merge has always behaved this way, which is the parity the change exists for.

The callable rule above moves an interface member out of `property` as well as into `function`, and the losing direction is the quiet one there too. A reference declaring `symbol: "property"` over a file of interfaces stops counting a member spelled `onOpen: () => void`, so a build that was red for leaving it uncited goes green with no code change and no message. A claim declaring `symbol: "property"` over interfaces whose members are all callables now selects no host at all, and a claim with no selected host is inactive, which drops its whole obligation silently. Both are the intended classification arriving in the direction that produces no diagnostic, so read a `property` selector over interface files as the place to check after upgrading: name the kinds the population really holds, or widen the selector to both.
...
Qualified identities keep their owner. `Orders.Input.id` is a property below `Orders.Input`, while `Orders.state` is namespace data. Exported object and array binding patterns expose each local binding leaf as a property, and a type-only alias exposes its public type-space descendants, including every member declared by an object-shaped type alias or by an interface no class merges with, while withholding value-space: namespace data, namespace functions, every class member, and the members of an interface that does merge with a class.
