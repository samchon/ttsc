...
In `docs/review.md`, cite `@link ../../api/src/example.ts#Target.property <reason>`. `root` selects a code population; it does not change the citation's relative-path base. Rooted references follow re-exports inside that root, and report exports that leave it or cannot be loaded. Choose a root containing the complete exported implementation. They do not search other directories for a matching filename or symbol. Active Program snapshots take precedence over the saved content of the same file.
