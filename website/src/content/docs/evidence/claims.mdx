...
- **`requireReview`** makes every acknowledgement here owe an [`@evidenceReview`](/docs/evidence/tags) naming the same target and carrying a fingerprint of the cited scope's content. When that content changes the fingerprint stops matching and the build fails again with the new value in the diagnostic. Expiry is the whole point: a review that cannot expire is written once and stays green forever, and on a second pass over a large citation set nothing tells you which reviews were written against content that has since moved. It claims no more than that, since the diagnostic states the expected value and nothing here judges whether prose is sincere.
