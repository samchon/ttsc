...
A positive citation answers the item it names and nothing beneath it, and a target naming no item at all is reported as an aggregate. Both halves are needed. Citing a heading that is an item and also contains items stays legal, and it answers that one heading; the headings under it are still owed. Under the default selector the document itself is an item, so refusing only the unnamed scope would leave one `@evidence docs/rules.md` cascading through every heading and ticking every box. The aggregate report is the complete statement of what to repair: the items the refused target covered are not listed again on that host's shortfall, because that diagnostic already names them and names the one repair. `@evidenceExclude` keeps the cascade, because "none of this applies here" is one reviewed decision however many items it covers, and a host that does not participate at all should not owe one tag per item to say so.

Duplicate and conflict detection moves to the host. Two hosts excluding one item, and one host citing an item another host excludes, are the expected state of a checklist rather than a duplicate and a contradiction. An exclusion written on a carrier that is not itself a selected host stays claim-wide: it records that no host owes that item, which is what gathering exclusions into one ledger file is for. The reach comes from the carrier, never from the file it sits in, so give a ledger a placeholder the claim's `symbol` does not select. A ledger whose placeholder is selected answers for that placeholder alone, and the placeholder then owes the checklist like any other host. A TypeScript claim has to narrow `symbol` for such a placeholder to exist at all, because its default selects every kind and every eligible carrier is then a selected host. A Prisma claim needs no placeholder, since an unattached top-level `///` run is a carrier that hosts nothing and is the ledger shape this was built around. A Markdown claim has none: its only eligible carrier is a selected host, so gather exclusions there by claim rather than by file.

`uniqueEvidence` and `singleEvidencePerSymbol` are refused beside it, at configuration time rather than as coverage failures. They are not merely redundant here. A checklist requires every host to cite every unit, which the first forbids the moment a claim has two hosts and the second forbids the moment the population has two units, and a graph with one host and one unit would satisfy all three by accident until the second file arrived.

Pair it with `requireReview` for the property a checklist is usually wanted for. Each host's acknowledgement then carries the fingerprint of that item alone, so editing one item expires every host's answer to it and nothing else. Under the aggregate behavior the option refuses, one citation carries one fingerprint for the whole document.
