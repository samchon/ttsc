...
A citation covers its target and every selected descendant. So `symbol: "property"` on a reference can still be satisfied by one `@evidence {@link IShoppingSale} ...`, and one `@evidence prisma:Sale ...` discharges every selected column and relation beneath that model. A reference can turn that cascade off with [`noAggregateEvidence`](#reference-policies), which is what a population whose citing hosts do not own whole subtrees wants.
