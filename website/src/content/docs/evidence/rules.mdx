...
One is the graph. The other four protect the conditions the graph depends on, and each is a file rule.
...
Requires an `@evidenceReview` beside every `@evidence` and `@evidenceExclude`, naming the same target. Takes no options.
...
A review is an annotation of a citation, never an acknowledgement of a unit. It discharges no coverage, contributes no host to `uniqueEvidence`, and counts toward no `singleEvidencePerSymbol` total. Enabling this rule cannot change an `evidence/graph` diagnostic.

Four states are judged, all inside one documentation block on one public identity. A citation with no review of its target is unreviewed. A review naming a target this identity does not cite is an orphan, which is what turns a mistyped target into a finding instead of a silent hole. A second review of one target is a duplicate, since one citation is verified once. A review with no description is malformed.

The unit judged is an identity rather than a declaration, so a review on `namespace ISale` answers a citation on `interface ISale`. A declaration withdrawn by `@internal`, `@hidden`, or `@ignore` owes nothing, because it can carry no citation the graph will read.

The rule does not judge whether the description is sincere, and it is not built to. A second prose block is as hedgeable as the first. What makes a review expire when the thing it reviewed moves is the optional `#`-prefixed fingerprint, which this rule carries without interpreting and [`requireReview`](/docs/evidence/claims) validates against the cited content.
