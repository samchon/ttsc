...
Then compare the AST discussion in [AST and Checker](/docs/plugin-development/concepts/tsgo#recognizing-calls).
...
Then compare [AST and Checker](/docs/plugin-development/concepts/tsgo#recognizing-imports-and-module-specifiers).
...
`@ttsc/lint` exposes a public Go module — `github.com/samchon/ttsc/packages/lint/rule` — that third-party packages import to register rules. ttsc's plugin builder statically links the contributor's Go source into `@ttsc/lint`'s binary via the protocol-level `contributors` field (see [Protocol: Contributors](/docs/plugin-development/concepts/protocol#contributors)), so contributor rules share the same single AST walk and diagnostic stream as the built-in corpus.
...
- [Architecture](/docs/plugin-development/reference/architecture) — the build/cache pipeline behind every plugin.
- [Pitfalls](/docs/plugin-development/reference/pitfalls) — failure modes specific to plugin authoring.
