...
## The real logic — `pathsRewriter` in `utility/host.go`

Everything below is in [`packages/ttsc/utility/host.go`](https://github.com/samchon/ttsc/tree/master/packages/ttsc/utility/host.go). Six steps:
...
`pathsRewriter` is the only first-party transform that needs the Program (banner and strip work off source files alone). Its constructor caches the Program's `ParsedConfig.CompilerOptions`, `SourceFiles`, and the `Paths` map; everything else is a pure function of those four facts and the input specifier string.
...
- The `optionalPath` / `normalizePath` / `stripKnownSourceExtension` helpers wholesale — they were tuned to match tsgo's exact behavior in `utility/host.go` and contain edge cases (Windows drive prefixes, `.d.ts` vs `.ts`) that are mostly noise to a third-party plugin. Re-derive them as you need them.
- The `prepareTransforms` dispatch-by-name. As with strip, this is first-party-only.
- Reading the first `--plugins-json` entry as "yours." The host sends every loaded plugin entry in `--plugins-json`, not just yours — first-party plugins co-host and dispatch by name. A third-party plugin owns its binary, so the entry whose `name` matches **your descriptor's `name`** is the one to read. Indexing `entries[0]` works until the user reorders their tsconfig `plugins` array, at which point the binary picks up another plugin's config without an error. Match by `entry.Name == "<your-plugin-name>"` explicitly. (Ordering is deterministic — `check`-stage first, then `transform`-stage, both in tsconfig order — see [Plugin protocol → `--plugins-json`](/docs/development/concepts/protocol#--plugins-json).)
...
The synthesize-flag invariant is locked by tests that assert on the printer output text after mutation — search for `lit.Flags` and `NodeFlagsSynthesized` in `packages/ttsc/test/utility/`.
