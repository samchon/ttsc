...
Identical shape to strip. The descriptor at `src/index.cjs` points at linked
`driver/` source, while `plugin/main.go` is a standalone utility dispatcher
with the same shape shown in the [banner walkthrough](/docs/development/walkthroughs/banner#the-standalone-sidecar-pluginmaingo).
Skip ahead to the real logic.
