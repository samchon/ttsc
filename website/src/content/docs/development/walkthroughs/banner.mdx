...
The factory takes a `TtscPluginFactoryContext` parameter but ignores it — banner has no factory-time decisions to make. Other plugins use it to specialise at descriptor creation time: `context.tsconfig` is the absolute path of the active tsconfig, `context.cwd` is the resolved project directory, and `context.plugin` is the original `compilerOptions.plugins[i]` entry (typed by your plugin's option interface). `@ttsc/lint`'s factory reads `context.plugin.configFile` to locate the lint config and discover contributor packages; banner has nothing analogous to do.
