...
`@ttsc/banner` is the simplest first-party plugin. It does one thing: insert a JSDoc `@packageDocumentation` block at the top of every emitted `.js` and `.d.ts` file. Go side-notes are heaviest on this page — subsequent pages assume the vocabulary it introduces.
...
└── plugin/
    ...
    └── banner.go             ← A 2-line marker file (see below)
```
...
`ttsc` builds `plugin/main.go` on the consumer machine the first time the plugin is loaded, then caches the binary. See [Architecture](/docs/development/reference/architecture) for the full cache key.
...
- `name: "@ttsc/banner"` — shown in errors and used by the host to identify this plugin in `--plugins-json`.
- `source: path.resolve(__dirname, "..", "plugin")` — absolute path to the Go command-package directory. `__dirname` is the directory holding the compiled `lib/index.js`; `..` steps up to the package root; `plugin` is the Go source folder.
...
The `import` block lists every package this file uses. `fmt` and `os` are part of Go's standard library; the third is the shared host that holds the actual banner-rendering logic.
...
Every first-party plugin's `main.go` uses this idiom. Copy it.
...
These three functions live in `packages/ttsc/utility/host.go` — the shared first-party host the three transform plugins co-host. The banner-specific code is inside that file, gated by the plugin name `@ttsc/banner` in the dispatch logic.
...
Two lines. The file exists because the Go build infrastructure expects every package directory to have at least one non-test `.go` file beyond `main.go` for the `files` array to make sense; the comment is for the future reader who opens the directory and wonders where the real code went.

When you write your own plugin you do **not** need this marker file. Banner's first-party shape inherits it from a convention internal to this monorepo.

## The actual transform logic — a brief tour

The full transform code lives in [`packages/ttsc/utility/host.go`](https://github.com/samchon/ttsc/tree/master/packages/ttsc/utility/host.go). For banner, the relevant slice is:

1. **`parsePluginEntries`** decodes the `--plugins-json` payload into a slice of `pluginEntry { Name, Stage, Config }`.
2. **`prepareSourcePreamble`** loops over those entries, picks out the ones named `@ttsc/banner`, and calls `parseBanner(entry.Config, cwd, tsconfigPath)` for each. The results are concatenated into one `sourcePreamble` string.
...
4. **`driver.LoadProgram(cwd, tsconfig, LoadProgramOptions{SourcePreamble: preamble, ...})`** loads the project Program with the preamble injected before parsing. From that point onward, every source file in the Program *already contains* the JSDoc block at offset 0 — the printer round-trips it naturally.
5. **`makeSourcePreambleWriteFile`** wraps `tsgo`'s emit `WriteFile` callback. If the emitted file does not already contain the preamble text (e.g. `.d.ts` files that tsgo generated from declarations rather than from the source text), the wrapper inserts it via `driver.ApplySourcePreamble`.
...
The hairy bit is loading TypeScript config files (`banner.config.ts`). Because the config is TypeScript and may use ESM imports, banner spawns `ttsx` (injected as `TTSC_TTSX_BINARY` — see [Architecture → Injected into the plugin process](/docs/development/reference/architecture#injected-into-the-plugin-process)) to evaluate it, then captures the result through stdout JSON. This is `loadBannerTypeScriptConfigFile` — about 60 lines of "synthesize a tempdir, link `node_modules`, write a tiny loader, run it, parse the JSON it prints." Worth reading once for the pattern; not something you typically reimplement.

### Sidebar: `defer prog.Close()` is a hard rule

`utility.RunBuild` hides the Program lifecycle behind a helper, but the moment you copy this shape with `driver.LoadProgram` (the [supported third-party path](/docs/development/reference/driver-api)), you own the leased Checker. Every `driver.LoadProgram` call must be paired with `defer prog.Close()` on the very next line:
...
- The `main` → `run` → `switch` shape. Every first-party plugin uses it; so do the canonical third-party fixtures.
...
- `utility.RunBuild / RunTransform / RunCheck`. These are hard-coded against the first-party plugin names (`@ttsc/banner`, `@ttsc/paths`, `@ttsc/strip`). A third-party plugin that imports them compiles cleanly and silently no-ops — there is no error to grep for. Call [`driver.LoadProgram`](/docs/development/reference/driver-api) directly, the way [`tests/projects/go-source-plugin-checker/plugin/main.go`](https://github.com/samchon/ttsc/tree/master/tests/projects/go-source-plugin-checker) does.
- The empty `plugin/banner.go` marker file. Third-party plugins do not need it.
- The dispatch-by-name pattern in `utility/host.go::prepareSourcePreamble`. A third-party plugin owns its own binary; it does not need to filter the `--plugins-json` entries by name to find "its" config. Read `plugins[0].Config` (or the entry whose `Name` matches your own descriptor) directly.
...
- [`packages/banner/test`](https://github.com/samchon/ttsc/tree/master/packages/banner/test/) — Go unit tests for the banner-specific helpers in `utility/host.go`, each one assertion per file.
- [`packages/ttsc/test/utility`](https://github.com/samchon/ttsc/tree/master/packages/ttsc/test/utility/) — end-to-end tests that spawn the real binary against fixture projects.
