...
The pattern is: **prefer source-level transformations over post-emit edits**. By overlaying the preamble at parse time, banner's own code never touches declaration emit or formatting, tsgo treats the preamble as ordinary source text and round-trips it into both `.js` and `.d.ts`. The one thing source-level injection costs is source maps: treating the preamble as real source shifts every recorded coordinate down by its line count, so the generic host corrects the emitted maps (`AdjustEmittedSourceMap` in `makeSourcePreambleWriteFile`) — see [Recipes → Source Maps](/docs/development/authoring/recipes#source-maps). The plugin author still writes none of that; the host owns it.
