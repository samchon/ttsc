...
// dist/index.js
/**
 * License MIT Copyright 2026 Example Corp
 *
...
`@ttsc/banner` walks upward from the tsconfig directory looking for `banner.config.{ts,cts,mts,js,cjs,mjs,json}`. To point at a specific file, set `configFile` on the tsconfig entry: `{ "transform": "@ttsc/banner", "configFile": "./path/to/banner.config.ts" }`.
...
This is the standalone utility entrypoint for the package. The published
`@ttsc/banner` descriptor points at `driver/`, so consumer builds link the
driver package into the selected native host. Reading `plugin/main.go` is still
useful because it shows the executable sidecar contract and is what the package
Go tests run directly. The entire content:
...
Executable sidecars are always spawned with a subcommand as the first argument:
`check`, `transform`, `build`, and for check-stage plugins also `fix` or
`format`. Banner's published descriptor uses linked `driver/` instead, so in
normal consumer builds the generic native host receives the subcommand and the
banner driver runs through its registered hooks. This standalone dispatcher
exists for package tests and as an executable-sidecar example.
...
3. **`parseBanner`** delegates to **`resolveBannerText`** for config resolution, then formats the resulting text into a JSDoc `/** ... @packageDocumentation */` block. `resolveBannerText` tries, in order:
   ...
   - upward filesystem walk from the tsconfig directory looking for any `banner.config.{ts,cts,mts,js,cjs,mjs,json}`.
