...
- **Discovery walk** when `configFile` is absent: walk upward from the tsconfig directory looking for `lint.config.{ts,mts,cts,mjs,cjs,js,json}`.
...
`files` and `ignores` scope the object when the resolver (`linthost/config.go`) builds the rule severity map for each `FileName()`.
...
- **End-to-end TypeScript tests** in [`tests/test-lint/src/features/`](https://github.com/samchon/ttsc/tree/master/tests/test-lint/src/features/) — over 200 cases, every rule pinned by at least one assertion. Subdirectories: `rules/`, `fix/`, `format/`, `contributor/`.
