...
ttscserver fetches the corpus once, in the background, and answers from memory. It merges its trigger characters into whatever tsgo advertised rather than replacing them, and appends its items to tsgo's completion response rather than answering in its place. Until the fetch lands the corpus is empty and the editor sees exactly what it sees without the plugin.
