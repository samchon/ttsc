...
| `format`     | `ttsc format`                            | Apply *only* format-class rule edits. Write-only — no diagnostics, no emit.                 |
| `build`      | `ttsc build`                             | Same as `check`, plus tsgo's emit pipeline if `--noEmit` is not set.                        |
| `transform`  | `@ttsc/unplugin` and programmatic API    | Single-file emit. Lint still runs over every user source file in the Program. No user-facing `ttsc transform` command — the host spawns this verb when a bundler adapter or `ttsc.transform()` API call needs single-file output. |
...
```jsonc
// tsconfig.json — point the plugin entry at the file
{
  "compilerOptions": {
...
packages/lint/
├── package.json
├── src/                      ← TypeScript-side: factory + types + defineConfig
│   ├── index.ts              ← createTtscPlugin factory (~800 LOC)
│   ├── defineConfig.ts       ← helper for typed lint.config.ts
...
│   ├── eslint_runtime.go     ← delegates @typescript-eslint rules to Node ESLint
...
Two things are different from the earlier first-party plugins:
...
2. **The "real logic" is reusable.** Unlike the `utility/host.go` allowlist, every symbol in `linthost/` is callable by anyone who imports the package. Third-party plugins that want the same "check + fix + format" surface can copy this pattern verbatim.
...
Banner, strip, and paths reach the Program through the shared `packages/ttsc/utility/host.go`. `@ttsc/lint` cannot — that file is gated by the first-party allowlist. Instead, it inlines a `loadProgram` function that mirrors the [canonical third-party bootstrap](/docs/development/concepts/tsgo#bootstrap-with-the-driver):
...
6. Runs the ESLint runtime delegation when the resolved config asked for it — see [ESLint runtime delegation](#eslint-runtime-delegation).
...
The config layer lives in `linthost/config.go` — the longest file in `linthost/` (~1700 LOC) because it owns every way a user can declare rules:
...
- **ESLint flat-config** in `eslint.config.ts` — including per-file overrides via the `files` glob array.
...
The engine queries this resolver per file, so two files with different overrides get different rule sets. The resolver also handles the `.gitignore`-like `ignores` field from flat-config, returning a `ResolvedRuleConfig` with `Ignored: true` for excluded paths so the engine can short-circuit without walking the AST.

For the TypeScript-side discovery (`lint.config.ts` evaluation through `ttsx`), see [`packages/lint/src/index.ts`](https://github.com/samchon/ttsc/tree/master/packages/lint/src/index.ts) — the JS factory spawns `ttsx` against a synthesized loader that extracts every `plugins` map from the user's flat config, then writes the result to stdout as JSON. The Go side parses that JSON and feeds the resulting `contributors` into the plugin descriptor.
...
**Severity is per-file.** `fileRules.Severity(name)` is queried at dispatch time, not engine bootstrap, because flat-config `files: ["src/**.test.ts"]` overrides shift the severity per file.
...
For the canonical "use the Checker" rule, read [`rules_promise.go`](https://github.com/samchon/ttsc/tree/master/packages/lint/linthost/rules_promise.go). Its `isThenableType` helper detects whether a type carries a callable `.then` member — exactly the structural test ECMAScript uses for `await`:
...
- `GetPropertyOfType(t, "then")` — look up a member by name. Returns `*Symbol` or nil.
- `GetSignaturesOfType(thenT, SignatureKindCall)` — get every callable signature on the property type.

If `then` is missing or non-callable, the value is not thenable and `await` on it is meaningless. The rule reports a finding; `--fix` could autosuggest removing the `await` (though `await-thenable` does not — that's a design choice, not a technical constraint).
...
The trigger is **opt-in by config shape**, not "unknown rule falls through to ESLint":
...
The harness is ESLint-version-aware — it adapts between ESLint 8 (legacy `.eslintrc.*`) and ESLint 9 (flat config). The key idea: a single `ttsc check` invocation can mix native Go rules with delegated Node rules and the user sees one diagnostic stream — but only when the user's config asked for it.

## Contributor adapter

The adapter layer lives in `linthost/contrib_adapter.go`. Third-party rule contributors use a **separate** Go package — [`github.com/samchon/ttsc/packages/lint/rule`](https://github.com/samchon/ttsc/tree/master/packages/lint/rule) — that defines a *public* `rule.Rule` interface, structurally identical to the internal one. The adapter wraps each contributor rule onto the engine's internal interface:
...
```jsonc
// tsconfig.json — inline plugins map
{
  "compilerOptions": {
    "plugins": [
      {
...
```ts
// lint.config.ts — flat-config plugins object
...
## `defineConfig` and per-file overrides

The TypeScript-side `defineConfig` is a typed factory for flat-config arrays:

```ts
import { defineConfig } from "@ttsc/lint";

export default defineConfig([
  // Base rules apply to every file.
  { rules: { "no-var": "error" } },

  // Override block: only files matching `files` glob get these rules.
  {
    files: ["**/*.test.ts"],
    rules: {
      "no-console": "off",
...
The array is iterated in order; later entries override earlier ones for matching files. The resolver (`linthost/config.go`) is the Go-side mirror of this iteration — for every file's `FileName()`, it builds the merged rule severity map by stepping through the array.
...
- **End-to-end TypeScript tests** in [`tests/test-lint/src/features/`](https://github.com/samchon/ttsc/tree/master/tests/test-lint/src/features/) — over 200 cases, every rule pinned by at least one assertion. Subdirectories: `rules/`, `fix/`, `format/`, `contributor/`, `eslint/`.
...
- [Architecture](/docs/development/reference/architecture) — the cache, the Go toolchain resolver, the `TTSC_*_BINARY` env vars `@ttsc/lint` itself relies on when delegating to ESLint.
