...
└── rule/                     ← public Go API for third-party rule contributors
    ...
    └── astutil/              ← byte-oriented AST helpers contributors can reuse
```
...
For more autofix shapes, [`rule/astutil`](https://github.com/samchon/ttsc/tree/master/packages/lint/rule/astutil) ships the byte-oriented helpers built-in rules already use: `NodeText`, `KeywordStart`, `FindKeyword`, `TokenRange`, and the comment enumeration `ForEachComment` with its `CommentToken` and the single-gap `ScanCommentGap`.
