...
The VS Code path also uses the LSP sidecar verbs because `@ttsc/lint` sets `capabilities.lsp`: `lsp-command-ids`, `lsp-code-action-kinds`, `lsp-diagnostics`, `lsp-code-actions`, and `lsp-execute-command`.
