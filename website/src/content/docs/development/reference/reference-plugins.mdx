...
- Token-oriented diagnostic ranges with `shimscanner.SkipTrivia(file.Text(), node.Pos())` — used by every `linthost/rules_*.go` file; the canonical idiom across the chapter.
