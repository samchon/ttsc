...
- `Edges` maps each file to its direct resolved references — imports, re-exports, `/// <reference>` targets, type reference directives, type-only edges included — exactly the per-file `referencedMap` entry tsgo's own incremental engine stores in `tsbuildinfo`.
...
- `Candidates` maps each importing file to missing paths that would outrank its selected module target. Bundlers watch and hash these paths so creating one reloads an otherwise unchanged program. Lower-priority paths are excluded.
