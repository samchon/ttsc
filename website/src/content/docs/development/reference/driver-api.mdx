...
A **synthetic** import is preserved unconditionally, since it has no parse-tree original for elision to reason about, but only while it stays synthetic. Elision tests the import clause, named specifier, or namespace import rather than the declaration, so `SetOriginal`-linking one of those to its parse-tree counterpart puts it back under that counterpart's marks. `EmitContext.ParseNode` walks the original chain before deciding.
