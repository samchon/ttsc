...
Import elision decides which of the file's own imports survive from linked references the checker marks, and the driver marks them on the parse tree — the file TypeScript-Go analyzed, not the tree your transform returns. Three consequences are worth knowing before you rewrite anything.
