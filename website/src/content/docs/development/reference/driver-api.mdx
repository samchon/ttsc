...
Synthesizing that import is the only way to reach a module from injected code. The driver marks linked references on the parse tree, before your transform runs, so a value reference your transform injects cannot keep an import alive that the source itself uses only in type position — TypeScript-Go's import elision has already decided that import carries no runtime value. Your own synthetic import is never at risk: it has no parse-tree original, so elision cannot reason about it and preserves it unconditionally. The reverse also holds. When your transform removes the last value use of an import the source really had, that import survives, because the marks describe the file the checker analyzed rather than the file you produced. This is the same contract `ts-patch` had on TypeScript 5 and 6, where the marks were fixed at check time.
