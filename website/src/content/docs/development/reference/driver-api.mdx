...
A plugin that discovers its own config by walking up from the project uses `driver.DiscoverConfigFile(base, names)`, which returns the match, the directory it stopped in, and every candidate it probed and did not find. Those rejected candidates decide the result as much as the match does, since one created nearer the entry wins the next search and one created beside the match makes that directory ambiguous, so report them with `driver.ReportMissingConfigCandidates(discovery.Probed, ctx.ReportHostInputHash, ctx.ReportHostInputRealpath)`. A consumer then invalidates when one appears, which a config living outside the project walk cannot otherwise cause.
