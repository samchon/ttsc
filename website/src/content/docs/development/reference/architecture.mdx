...
Source: `packages/ttsc/src/plugin/internal/buildSourcePlugin.ts` (`GO_BUILD_ENV_KEYS` constant; the cache-key loop folds each non-empty effective value into the hash).
