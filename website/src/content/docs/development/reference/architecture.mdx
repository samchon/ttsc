...
### Injected into the plugin process: `TTSC_*_BINARY`

Before spawning a plugin, `ttsc` sets three env vars in the child environment so the plugin can re-spawn the ttsc toolchain without resolving paths itself:
...
- [Reference plugins](/docs/development/reference/reference-plugins) — first-party plugins as worked examples.
