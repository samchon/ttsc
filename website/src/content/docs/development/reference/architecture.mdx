...
6. Every set env var in `GO_BUILD_ENV_KEYS`: `GOOS`, `GOARCH`, `GOAMD64`, `GOARM`, `GO386`, `GOMIPS`, `GOMIPS64`, `GOPPC64`, `GORISCV64`, `GOWASM`, `GOFLAGS`, `GOEXPERIMENT`, `CGO_ENABLED`, `GOTOOLCHAIN`, `GOROOT`.
...
These 15 Go build env vars fold into the cache key when non-empty, so cross-compile targets, build-tag tweaks, and explicit toolchain roots never collide with the native build slot:

```text
GOOS GOARCH GOAMD64 GOARM GO386 GOMIPS GOMIPS64 GOPPC64 GORISCV64
GOWASM GOFLAGS GOEXPERIMENT CGO_ENABLED GOTOOLCHAIN GOROOT
```
