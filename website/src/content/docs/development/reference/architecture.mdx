...
6. Classify the source package. A `package main` source is an executable
   sidecar. A non-`main` transform source is a linked plugin package that must
   register through `driver.RegisterPlugin`.
7. For executable sidecars, run `go build -o .ttsc-plugin <entry>` in the
   scratch directory. The leading-dot placeholder name makes the in-progress
   binary easy to spot in `go build` stderr; the cache move renames it to
   `plugin` (or `plugin.exe` on Windows).
8. For linked transform sources, synthesize or select a native host that
   blank-imports the copied package, then build that host binary instead.
9. Move the built binary into the cache.
10. Invoke executable sidecars with `check`, `transform`, `build`, `fix`, or
    `format`. Linked transform packages do not receive argv directly; the
    native host receives the subcommand and dispatches to registered driver
    hooks.
