...
Create one context per filesystem transaction and reuse it for every comparison in that transaction. The context memoizes each path's realpath and directory case decision when that input is first resolved, which keeps repeated comparisons consistent and avoids repeated filesystem probes. It is not an atomic filesystem snapshot: a different path first resolved later can observe a newer filesystem state.

Existing path segments use their physical spelling. Missing suffixes preserve spelling when the nearest existing directory is proven case-sensitive and fold it when that directory is proven case-insensitive. Windows drive and UNC roots are always matched case-insensitively. If the directory behavior cannot be proven, Windows and macOS use their case-insensitive platform default while other platforms assume case-sensitive behavior. An empty, unreadable, or otherwise unprovable NTFS/APFS directory that was explicitly configured as case-sensitive can therefore merge case-only missing suffixes until a probe can establish its behavior.
