...
Create one context per filesystem transaction and reuse it for every comparison in that transaction. The context caches realpaths and directory case semantics so one scan cannot compare paths from different filesystem snapshots. Existing path segments use their physical spelling. Missing suffixes preserve spelling under a case-sensitive directory and fold it under a case-insensitive directory. Windows drive and UNC roots are always matched case-insensitively, while descendant segments follow the directory's configured case semantics.
