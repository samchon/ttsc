...
`package.json#ttsc.plugin` is read only from packages listed directly in the nearest consumer `package.json` at or above the selected project. The plugin's `package.json` `name` must equal the consumer's dependency string exactly — auto-discovery matches them character for character.
...
`@ttsc/banner` and `@ttsc/strip` are first-party transform plugins that share `packages/ttsc/utility/host.go` under a closed allowlist.
...
This fails — two unrelated third-party transform binaries cannot share one source-to-source pass:
...
The first fires during `ttsc build` / `ttsc check`; the second during `ttsc transform`. The fix is the same: pick one aggregate plugin and list the others in its `composes: [...]` array. See [Composition failures](#composition-failures) below for the per-rule error strings.
...
`node.Pos()` includes leading trivia. For token starts use `shimscanner.SkipTrivia` — this is the idiom every rule in `packages/lint/linthost/` uses:
...
Skipped: `node_modules`, `.git`, `.ttsc`, `go.work`, `go.work.sum`, archive files, OS metadata (`.DS_Store`, `Thumbs.db`), and editor backup files ending in `~`. Consumer TypeScript source files and plugin options such as lint `config`, `banner`, or `calls` do not affect the source-plugin binary cache.

The cache also folds in the Go binary's identity and the 14 `GO_BUILD_ENV_KEYS` env vars — see [Architecture → Cache key inputs](/docs/development/reference/architecture#cache-key-inputs). Pinning `TTSC_GO_BINARY` to a fixed absolute path makes the slot stable across `pnpm install`.
...
The first-party utility plugins implement `version` as a sanity check (each prints a build-id line) so a maintainer can `go run ./plugin version` to confirm the binary is wired up. `ttsc` itself never invokes `version`. Plugin authors who skip `version` lose the smoke-test path but break nothing on the host side.
...
- [Reference plugins](/docs/development/reference/reference-plugins) — worked examples of the patterns that work.
