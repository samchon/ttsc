...
`@ttsc/banner`, `@ttsc/paths`, and `@ttsc/strip` are the three first-party transform plugins that share `packages/ttsc/utility/host.go` under a closed allowlist (`FIRST_PARTY_UTILITY_PLUGIN_NAMES`). Any combination of them composes inside one compiler emit pass; mixing in a third-party transform plugin forces a separate spawn and triggers the abort below.
...
For the full cache-key formula and the env vars that enter it (which is why `pnpm install` can invalidate the slot), see [Architecture → Cache key inputs](/docs/development/reference/architecture#cache-key-inputs). To force a cold rebuild:
