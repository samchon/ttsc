...
The first fires during `ttsc build` / `ttsc check`; the second during `ttsc transform`. The fix is the same: pick one aggregate executable plugin and list the others in its `composes: [...]` array, or make the additional transforms linked non-`main` packages. See [Composition failures](#composition-failures) below for the per-rule error strings.
