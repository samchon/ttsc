...
A host with a persistent cache restores a module by the dependencies it recorded, without running the adapter. The module's dependencies are itself and the record, so the record must move exactly when the project's state does, and only a delivery or a proof can move it.

- `bridge/writeProjectRecordFile.ts` writes a record only when its bytes would change, with keys in one order, so two generations of one state produce one byte sequence and no delivery moves a record for nothing.
- `bridge/refreshProjectRecordFiles.ts` runs before a host validates anything against its cache: `buildStart` in `unplugin.ts`, esbuild's `onStart`, `withTtsc` before Next starts either compiler, and the Turbopack loader's first run in a process. It proves every recorded input against the disk the way a delivery proves a generation (`watch/watchInputEvidenceMatchesDisk.ts`: a predicate observation by every predicate it holds, a graph input by its state hash and physical target, a host input by its bytes' hash), walks the project again under the recorded policy (`bridge/membershipRecordDigest.ts`), and moves a record whose state changed while nothing ran, or whose tsconfig is gone. A moved record keeps moving at each start until a delivery writes the state the proof found.
