...
Next runs three webpack compilers over one project, and the wrapper gives each its own plugin instance, so each opens a bridge of its own and three observers watch one project's inputs. On Linux a directory's watch is shared process-wide, so the kernel holds one watch however many observers ask for it; the redundancy is bookkeeping and duplicate signals, which each registration settles. Sharing one bridge per root would need a lease across compiler shutdowns, as the transform cache already has.
