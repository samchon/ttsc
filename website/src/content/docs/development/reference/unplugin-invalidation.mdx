...
`bridge/registerBuildWatchInputs.ts` routes each input by `watch/classifyWatchInput.ts` to the host's channel (file, missing, listing) or to the bridge for the kinds the host cannot observe; the membership goes only to a bridge. A missing path is handed to the host's missing channel with what may appear there (`watch/missingWatchInputShape.ts`: a directory, a file, or either when the registration carries no evidence, as a failed compile's does). Turbopack has no missing channel, and its file channel reads each registered path once the loader has returned, failing on a directory; that failure fails the module's evaluation, and Turbopack returns the worker to its pool with the loader's result still unread, so the next module on that worker receives this one's code. Its loader therefore keeps every missing path that may come back as a directory on the directory channel, whose read never fails. Rolldown and Rspack lose a change that lands in a window of their own, while building and before the watcher's baseline after a build, so every input of theirs reaches the bridge as well and the bridge confirms delivery, as for Turbopack. `vite/createViteServeInputWatch.ts` is the dev server's equivalent and the bridge's engine: `replace(importer, inputs, failed, startedAt)` re-checks each input against changes since `startedAt` and signals again at once when the delivery read a state already gone; a failed registration keeps the importer's earlier inputs observed.
...
A host with a persistent cache restores a module by the dependencies it recorded, without running the adapter, so every registration that must survive a restart has to be a file whose state moves exactly when the verdict does.

- The bridge's sentinels live in one directory for every process, `<tool>/watch-bridge` (`bridge/WATCH_BRIDGE_SENTINEL_DIRECTORY.ts`), and stay when the bridge closes. A registration never writes an existing sentinel; only a signal rewrites it, with bytes no earlier write left. A per-process sentinel directory made every restored module stale on every restart.
- The project's root-file membership reaches a persistent cache as a record file, `<tool>/membership/<hash>.json` (`bridge/membershipDigestFile.ts`), registered through the host's file channel on every delivery (`watch/projectMembershipRecordInput.ts`) and written only when its digest moves (`bridge/writeMembershipDigestFile.ts`, `bridge/membershipRecordDigest.ts`). Each build start refreshes every record by walking its project again (`bridge/refreshMembershipDigestFiles.ts`): `buildStart` in `unplugin.ts`, esbuild's `onStart`, and `withTtsc` before Next starts either compiler.
...
- The per-process directories beside them (`WATCH_BRIDGE_DIRECTORY_PREFIX`) hold only what may not outlive a process: the Turbopack marker for modules with untracked inputs, and the broker's probes.

## Where the host matrix measures this

`experimental/test-unplugin/src/contracts` drives every host through the scenarios in `scenarios.mjs` with two plugins, one whose envelope carries no graph and one whose compile runs the program, and the restart contract in `restarts.mjs` over each host's persistent cache, each session in a process of its own, where a restart over an unchanged project must compile nothing. A failure there names the host, the root spelling, the plugin, and the scenario; find its principle above, then its owner.
