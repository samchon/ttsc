...
The `Config` map is the consumer's tsconfig entry passed through verbatim — every key the consumer wrote, including ttsc-owned `transform` and `enabled` (ignore those; ttsc owns them). Plugin-owned options such as `mode`, `calls`, or `text` live inside `Config`, not beside `name` and `stage`.

The array is deterministically ordered: all check-stage entries first, then all transform-stage entries, each in their tsconfig order. A plugin that needs to know its position can rely on this.
...
```jsonc
// Explicit — wins over auto-discovery for the same package.
// Use this in fixtures, tests, and any case where the wiring should be unambiguous.
{
  "compilerOptions": {
...
```jsonc
// Auto-discovery — ttsc reads the plugin's package.json#ttsc.plugin
// when the package appears in the consumer's dependencies / devDependencies.
{
  "ttsc": { "plugin": { "transform": "my-ttsc-plugin" } }
}
```

Three rules to copy-paste:

1. **Explicit wins.** A `compilerOptions.plugins[]` entry with the same `transform` overrides the auto-discovered entry.
2. **`name` must match the dependency string.** Auto-discovery matches the consumer's `dependencies` / `devDependencies` entry against your plugin's `package.json` `name` field exactly. If they differ, ttsc loads nothing and silently emits unchanged.
3. **Only directly-listed packages.** ttsc reads `ttsc.plugin` only from packages listed directly in the nearest consumer `package.json` at or above the selected project — transitive dependencies do not auto-discover.

Source: `packages/ttsc/src/plugin/internal/loadProjectPlugins.ts::discoverPackagePluginEntries`.
...
func runCheck(opts options) int {
    ...
    exitCode := driver.WritePrettyDiagnostics(os.Stderr, diags, opts.cwd)
...
For diagnostic ranges that point at the first significant token (not leading trivia), use `shimscanner.SkipTrivia(file.Text(), node.Pos())` instead of `node.Pos()`. This is the idiom every rule in `packages/lint/linthost/` uses.
...
```go
// Source: packages/ttsc/utility/host.go::pathsRewriter (the @ttsc/paths leaf-text path)
lit.AsStringLiteral().Text = rewritten
lit.Flags |= shimast.NodeFlagsSynthesized
lit.Loc   = shimcore.UndefinedTextRange()
```

Structural mutations do **not** need this — the printer iterates `Statements.Nodes` directly without reading source spans, which is why `@ttsc/strip`'s statement filter (`packages/ttsc/utility/host.go::stripRewriter`) round-trips without the synthesize stamp.

See [AST & Checker → Mutating leaf text](/docs/development/concepts/tsgo#mutating-leaf-text-synthesize--reset-loc) for the underlying mechanism.

## Re-spawn `ttsx` or `tsgo` from a plugin

`ttsc` injects three env vars into every plugin process so plugins can re-enter the toolchain without resolving paths themselves:

| Var                  | Resolves to                                                   |
| -------------------- | ------------------------------------------------------------- |
| `TTSC_NODE_BINARY`   | Absolute path to the Node binary that launched `ttsc`.        |
| `TTSC_TSGO_BINARY`   | Absolute path to the resolved `tsgo` native binary.           |
| `TTSC_TTSX_BINARY`   | Absolute path to the `ttsx` launcher.                          |

Sources: `packages/ttsc/src/launcher/internal/runBuild.ts` and `packages/ttsc/src/compiler/internal/transformProjectInMemory.ts`.
