...
### Runtime-library packages: the `ttsc` export condition

When the plugin ships in a package whose main `.` entry is a **runtime barrel** — a library that re-exports its whole runtime (e.g. `typia`, whose index pulls in the validator runtime) — that entry cannot double as the plugin descriptor. `ttsc` loads the descriptor during plugin bootstrap, _before_ it builds the sidecar binary, so evaluating the barrel drags the runtime in (and, for a self-hosting transform, deadlocks: loading the transform would have to build the runtime the transform emits).

Expose a **runtime-free** descriptor under a dedicated `ttsc` export condition. `ttsc` honours it only when resolving a plugin entry, so the package's normal imports keep resolving to the runtime:
