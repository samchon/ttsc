...
Declare a transform plugin descriptor that points at an executable
`package main` source:
...
For a transform that should run inside the selected native host, point the
descriptor at a non-`main` package:
...
The linked package does not receive argv directly. It registers hooks from
`init()` and the native host invokes those hooks during `check`, `transform`, or
`build`:
...
Use this shape when your plugin only needs the shared transform hooks and does
not need a custom executable process.
