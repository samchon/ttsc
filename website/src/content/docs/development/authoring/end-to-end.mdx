...
After this works, compare with the shipped [`@ttsc/strip`](https://github.com/samchon/ttsc/tree/master/packages/strip/) and [`@ttsc/banner`](https://github.com/samchon/ttsc/tree/master/packages/banner/) plugins — note that those sidecars are 36-line dispatchers over a closed first-party helper (`packages/ttsc/utility/host.go`). Third-party plugins use the shape on this page, not the first-party shape. See [Reference plugins → Where the real logic lives](/docs/development/reference/reference-plugins#where-the-real-logic-lives).
...
The `files` field includes the Go source because `ttsc` builds it on the consumer machine. Published plugin manifests keep `ttsc` and `@typescript/native-preview` out of package dependencies; the consumer project supplies the active host and TypeScript-Go runtime. For the `peerDependencies` pattern that pins a compatible `ttsc` range, see [Publishing](/docs/development/authoring/publishing#pin-against-ttsc).

The `name` field must equal the consumer's dependency string — `ttsc`'s auto-discovery matches them exactly.
...
- This sample mutates **statement lists** (structural mutation). For mutations that rewrite the text of leaf nodes (identifiers, string literals, numeric literals), the printer reads the *original* source text via tsgo's `getTextOfNode` and the rewrite silently vanishes — see [Recipes → Mutating leaf-text nodes](/docs/development/authoring/recipes#mutating-leaf-text-nodes) for the synthesize-flag invariant.
...
For production-quality versions of this shape, read [`packages/strip`](https://github.com/samchon/ttsc/tree/master/packages/strip/) and [`packages/banner`](https://github.com/samchon/ttsc/tree/master/packages/banner/) — but note they share the first-party utility host (`packages/ttsc/utility/host.go`) and are not a template for third-party plugins.
