...
After this works, compare with the friendly walkthroughs of [`@ttsc/banner`](/docs/development/walkthroughs/banner) and [`@ttsc/strip`](/docs/development/walkthroughs/strip) — note that those sidecars are 36-line dispatchers over a closed first-party helper (`packages/ttsc/utility/host.go`). Third-party plugins use the shape on this page, not the first-party shape. See [Walkthroughs → Where the real logic lives](/docs/development/walkthroughs#where-the-real-logic-lives).
...
For production-quality versions of this shape, read the friendly walkthroughs of [`@ttsc/strip`](/docs/development/walkthroughs/strip) and [`@ttsc/banner`](/docs/development/walkthroughs/banner) — but note they share the first-party utility host (`packages/ttsc/utility/host.go`) and are not a template for third-party plugins. The [`@ttsc/paths` walkthrough](/docs/development/walkthroughs/paths) is the next step up in difficulty, and the [`@ttsc/lint` deep dive](/docs/development/walkthroughs/lint) is the right reference once your plugin grows past one file.
