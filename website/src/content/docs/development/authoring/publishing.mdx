...
- every Go source directory referenced by your descriptor (`plugin/` for an
  executable sidecar, `driver/` or another non-`main` package for linked
  transform source);
- `go.mod`;
- `go.sum`, when present;
...
The tarball contains Go source, not a prebuilt plugin binary. `ttsc` builds that
source on the consumer machine: executable `package main` sources become
sidecars, while linked non-`main` transform packages are compiled into the
selected native host.
