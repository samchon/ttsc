...
For publishing this repository's `ttsc`, `@ttsc/*`, and platform packages, see [Contributing → Workspace release](/docs/contributing/workspace-release) (maintainer-only).
...
**The `name` field is load-bearing.** ttsc's auto-discovery (`loadProjectPlugins.ts::discoverPackagePluginEntries`) matches the consumer's `dependencies` / `devDependencies` entry string against your plugin's `package.json` `name` exactly. If the consumer writes `"my-ttsc-plugin"` but your `package.json` says `"my-plugin"`, ttsc loads nothing, emits unchanged, and exits 0 — no error, no diagnostic. The `name` must equal the string consumers will install you under.
...
The tarball contains `plugin/main.go`; `ttsc` builds that source on the consumer machine.
...
For GitHub Actions or any CI that runs your test matrix:

- `TTSC_GO_BINARY` — absolute path to the Go binary `ttsc` should use to build plugins from source. Without it `ttsc` resolves through `setup-go` / `$PATH` / the bundled SDK; pinning makes the build cache deterministic. Set in `.github/workflows/test.yml:40-41` for this repo.
- `TTSC_CACHE_DIR` — per-worker isolation when matrix runs in parallel. See [Testing → Cache isolation](/docs/development/authoring/testing#cache-isolation).
