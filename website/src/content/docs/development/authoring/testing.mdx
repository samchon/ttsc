...
func Test_parsebanner_returns_configured_text_from_explicit_config_path(t *testing.T) { /* ... */ }
```
...
```go
// Source: packages/ttsc/utility/host.go::parseBanner
...
The `--plugins-json` flag is part of the public protocol (see [Plugin protocol → `--plugins-json`](/docs/development/concepts/protocol#--plugins-json)). The host serializes the active plugin descriptors as a JSON array — `check` plugins first, then `transform` — and each entry carries `{config, name, stage}`. `config` is the consumer's tsconfig entry passed through verbatim (including any `transform` or `enabled` keys; plugins should ignore those — ttsc owns them).

```json
[
  {
    "config": { "header": "// generated" },
    "name": "@ttsc/banner",
    "stage": "transform"
  }
]
...
Smoke from a Go test by shelling your binary directly with a hand-constructed manifest:
...
The host emits flags in equals form (`--cwd=...`); construct your tests the same way.

## Asserting re-spawn env vars

`ttsc` injects `TTSC_NODE_BINARY`, `TTSC_TSGO_BINARY`, and `TTSC_TTSX_BINARY` into every plugin process (see [Architecture → Injected into the plugin process](/docs/development/reference/architecture#injected-into-the-plugin-process--ttsc__binary)). To pin that your plugin receives them, ship one Go unit test that asserts on the env from inside the plugin's own `runCheck` / `runTransform`:
...
func Test_plugin_receives_ttsc_tsgo_binary(t *testing.T) { /* ... */ }
```
...
Plugin builds are cache-keyed by `ttsc + tsgo + platform + entry + plugin source hash + overlay hashes + the resolved Go binary's identity + every env var in `GO_BUILD_ENV_KEYS` (`GOOS`, `GOARCH`, `GOFLAGS`, `GOEXPERIMENT`, `CGO_ENABLED`, `GOTOOLCHAIN`, …). Three knobs make tests deterministic:

- **`TTSC_CACHE_DIR` per worker**, not per suite. Parallel workers that share a cache dir will race the publish path.

  ```ts
  const cache = fs.mkdtempSync(path.join(os.tmpdir(), `ttsc-plugin-cache-${workerId}-`));
  process.env.TTSC_CACHE_DIR = cache;
  ```

- **Freeze `GOOS` / `GOARCH` to the host.** A developer who exports `GOARCH=arm64` in their shell will rebuild plugins per test run because cross-compile env vars enter the cache key.
- **Pin `TTSC_GO_BINARY`** to a fixed absolute path. The cache key folds in `resolveGoCompilerIdentity` (resolved path + stat + `go version` text); pinning the binary makes the slot stable across `pnpm install`.
...
For this repository's full test matrix and release smoke checks, see [Contributing → Workspace release](/docs/contributing/workspace-release).
