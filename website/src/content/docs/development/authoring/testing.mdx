...
Plugin builds are cache-keyed by `GO_BUILD_ENV_KEYS` and the resolved Go binary's identity — see [Architecture → Cache key inputs](/docs/development/reference/architecture#cache-key-inputs) for the full formula. Three knobs make tests deterministic:
...
- **Pin `TTSC_GO_BINARY`** to a fixed absolute path so the cache slot stays stable across `pnpm install`.
