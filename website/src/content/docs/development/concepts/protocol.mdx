...
- `inputProofFailures`: optional machine-readable reasons why a realized graph member has no paired proof. `driver.NewTransformGraph` emits bounded stable codes such as `content-changed`, `content-unavailable`, `kind-changed`, `realpath-changed`, or `unobserved`; it does not emit entries for speculative candidates that the compiler was never expected to read. Consumers still treat the absent proof as a refusal, but can identify the producer condition and exact path instead of reducing every case to “proof missing.” This field is diagnostic evidence only and never authorizes reuse.
