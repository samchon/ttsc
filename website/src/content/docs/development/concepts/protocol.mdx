...
All entries are normalized absolute local filesystem patterns. `files` are retained while missing; `globs` describe populations and are retained while matching nothing. The command declares topology without loading a TypeScript Program or reading the external content. Hosts call it only when the capability is explicitly enabled, so older strict sidecars are never probed with an unknown command. A failing refresh leaves the consumer's last successful snapshot in force. HTTP(S) resources do not belong in this protocol.
...
| `lsp-serve` | Optional. Answer a newline-delimited stream of `lsp-diagnostics` / `lsp-code-actions` / `lsp-hints` requests over stdin, holding a warm Program across them. |
...
The `lsp-diagnostics` result is `{ "document": [...], "project": { "uri": "file:///logical/tsconfig.json", "diagnostics": [...] } }`; omit `project` when the sidecar owns no project result. `ttscserver` merges document diagnostics with upstream diagnostics, but publishes the project set separately at its config URI with a zero range and no document version. A newer saved-document generation replaces the prior set, clears an old URI before publishing a new one, and suppresses stale asynchronous results. Project diagnostics do not participate in code actions or fixes.
