...
The CLI watcher treats either reload lane as a cold execution transition instead of a warm external-data update. `ttscserver` dynamically registers both the exact reload-directory identity and its immediate children, then compares the current topology fingerprint before ending the native host. Before an intentional close it sends the `ttsc/pluginSelectionChanged` notification with `{ "reason": "projectInputChanged" }`; clients restart the JavaScript launcher as an expected lifecycle transition so descriptor discovery and contributor compilation run again without spending a crash-restart budget. Ordinary project inputs and unchanged reload-directory content continue through resident invalidation.
