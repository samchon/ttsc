...
| `TTSC_ROOT_FILES` | JSON array of absolute file paths that replace the tsconfig's file list: the program is built from exactly these files, with every option of the config parsed where it lives. `ttsx` sets it for a file the project does not list, such as an entry outside `include` or a TypeScript file inside an installed package. `driver.LoadProgram` applies it whenever the caller passed no explicit `LoadProgramOptions.RootFiles`, and it drops the config's TS18002/TS18003 diagnostics about its own list. A plugin that builds its own Program should read it through `driver.RootFilesFromEnv`, replace `ParsedConfig.FileNames` with it, and clear `ParsedConfig.ProjectReferences`; otherwise it compiles the project's own files instead of the file `ttsx` runs. |
...
`TTSC_TSGO_ARGS`, `TTSC_ROOT_FILES`, and `TTSC_PLUGIN_CONFIG_DIR` describe one invocation, so a spawn that has nothing to say clears whatever an ancestor `ttsc` process left behind rather than passing it on: a nested build never inherits the outer run's forwarded flags, root files, or project anchor. The two binary paths are the opposite — an inherited value wins, because it is how an embedder pins the toolchain for a whole process tree.
