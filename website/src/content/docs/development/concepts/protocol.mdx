...
- **Configs stay universal.** `graph.configs` remains an input of every file, listed or not. Compiler options reach generated code through the host, not through any file a plugin reads, so no consulted-declaration list can represent them.
