...
All entries are normalized absolute local filesystem patterns. `root` must resolve to the same physical filesystem identity as `--cwd`; consumers reject a snapshot rooted in a different project instead of redirecting a long-lived watcher. `files` are retained while missing; `globs` describe populations and are retained while matching nothing. Optional `reloadFiles` are exact paths whose change can alter plugin or contributor selection. The CLI watcher treats them as cold execution transitions instead of warm external-data updates. Producers also publish those paths in `files`: this preserves older decoders and lets `ttscserver` continue registering its existing file watches while it ignores the optional field. `reloadFiles` is CLI cold-selection metadata, not a separate LSP watch surface.
