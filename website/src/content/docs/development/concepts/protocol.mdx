...
- `candidates`: per importer, resolution probes that precede its selected module target in TypeScript-Go's resolution order. A probe can be absent or already exist without resolving; its creation or change can change an unchanged import's meaning, so consumers watch and hash it. Paths below the selected target are omitted and do not invalidate a cache.
