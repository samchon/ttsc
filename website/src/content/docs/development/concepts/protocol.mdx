...
- `hostInputs`: optional absolute paths whose content or presence universally affects the descriptor or native transform. Missing paths are valid resolution candidates. Ttsc already records tsconfig ancestry, package manifests inspected by auto-discovery, the descriptor's loaded CommonJS graph, and an explicit plugin `configFile`; use this field for implicit config discovery or other native inputs outside the TypeScript reference graph. For a bare specifier the descriptor resolves, the loaded graph holds the candidates of each `node_modules` search root up to the one whose package the resolution selected: the lookup never reads the roots after it, so a missing candidate there cannot change the descriptor, and a busy directory above the project, such as a home directory, cannot withdraw the descriptor's cache proof. A `#` specifier whose package `imports` maps it to a bare package holds that package's candidates the same way, from the importer up to the root that selected it. A linked native plugin that discovers inputs only while it evaluates configuration calls `PluginContext.ReportHostInput(absolutePath)` instead. Native transform envelopes return that generation-wide union and the JavaScript host merges it with descriptor inputs. The plugin's own Go source needs no report: the build keys the binary on it and on the Go build environment, and every transformation reports the directories it keyed on, with the state of each, sources and environment together, as `pluginSources`.
- `hostInputHashes`: optional evaluation-time fingerprints for `hostInputs`. Use a lowercase SHA-256 digest for an observed file and `null` for a missing candidate. An existing directory candidate is fingerprinted as the SHA-256 of the directory-kind marker `ttsc:host-input:directory` followed by one NUL byte, the digest ttsc-generated resolution loaders and the Go driver report for a directory, so replacing that directory with a file invalidates the generation. Capture the fingerprint when the descriptor reads or probes the input, before module/config resolution can select a result. Ttsc still watches an input without a fingerprint, but persistent bundler adapters decline narrow cache reuse because they cannot prove which state produced the descriptor. A linked native plugin reports the same evidence with `PluginContext.ReportHostInputHash(absolutePath, &digest)` or a nil digest for a missing candidate; contradictory observations keep the path in `hostInputs` and omit its fingerprint.
...
Before the host invokes any transform plugin, `assertSharedHostCompatibility` (`packages/ttsc/src/compiler/internal/sharedHost/assertSharedHostCompatibility.ts`) checks the resolved plugin binaries after linked packages are removed from the compiler-owner set. If more than one distinct executable binary remains active, the host aborts with one of:
...
```json
{
  ...
  "sourceMaps": {
    "src/main.ts": {
      ...
      "sources": ["main.ts"],
      "names": [],
      "mappings": "AAAA,MAAM,CAAC,MAAM,KAAK,GAAG,CAAC,CAAA",
  ...
  "graph": {
    ...
    "resolutionInputs": ["node_modules/@types"],
    "inputObservations": { "src/generated.ts": { "fileExists": false }, "node_modules/@types": { "accessibleEntries": { "directories": [], "files": [] }, "directoryExists": true } },
...
`sourceMaps` is optional: per file keyed like `typescript`, a version 3 source map from that text back to the text the file was transformed from. `sources` are relative to the file's directory, and `sourcesContent` must carry the text each source was transformed from. `@ttsc/unplugin` hands a module's map to the bundler only when that entry equals the text the bundler delivered, because a map of any other text would point at the wrong lines. Without an entry, a bundler reports positions in the transformed text, and Rollup warns `SOURCEMAP_BROKEN`. The built-in hosts supply a map for every file whose text differs from its source. The printer records it while printing: `shimprinter.EmitSourceFileWithSourceMap(shimprinter.PrinterOptions{InlineSources: true}, shimprinter.PrintHandlers{}, nil, file)` returns the text and the map together. It builds its own printer for each file, because a printer's source-map state carries across files. A host that inserts a source preamble, as the utility host does for `@ttsc/banner`, must also move the map back to the author's text, as `driver.Program.AuthoredSourceMap` does. Malformed entries are dropped, and a file without an entry keeps working without a map.

`dependencies` is optional advisory metadata: per transformed file (same keys as `typescript`), the source files whose content influenced that output beyond the file itself — for a type-driven generator, the declaration files it read through the Checker. Paths may be project-relative or absolute; the host passes them through verbatim on `TtscCompiler.transform()` results, and `@ttsc/unplugin` registers them as bundler watch files so type-only inputs invalidate the module in HMR. Dev-mode bundlers erase type-only imports from their module graph, so without this field an edit to a consulted type serves stale generated code until a cold restart. Omit the field (or a file's entry) when there is nothing to report; malformed entries are dropped, never fatal. A producer whose list is exhaustive for a file can additionally declare it [complete](#dependency-completeness), which narrows invalidation instead of widening it. A reported path outside the project that no graph proves carries no state from the compile, so `@ttsc/unplugin` keeps the output only when a reading of the path taken before the compile still matches it afterwards; a path the compile reports for the first time is compiled once more with that reading taken, so output produced from an earlier state of the file is never kept under a newer one.
...
- `candidates`: per importer, the exact lexical filesystem inputs consulted by a fresh replay of the pinned TypeScript-Go module, type-reference, and triple-slash path resolvers. The replay uses the resident resolution mode, compiler options, project-reference redirect, package conditions, and upstream resolver order, then excludes only an identically spelled selected graph target. This retains package manifests, failed predicates, unresolved searches, self-name and `#imports` package scopes, arbitrary-extension probes, and a selected symlink or junction spelling without maintaining a second resolver in ttsc. A host with nothing to report leaves the key out entirely rather than sending an empty object, and `TtscCompiler.transform()` decodes it the same way.
- `resolutionInputs`: automatic type discovery and type-reference resolution inputs that affect every source file. This includes each effective type-root predicate and its accessible child-directory listing, so adding a new `@types` package changes the generation even when no importer or existing declaration changed.
- `inputObservations`: optional predicate-preserving compiler-time proof for graph members. An entry contains only filesystem operations the compiler performed or identity reads captured beside a successful predicate: `fileExists`, `directoryExists`, `stat`, `readFile`, `realpath`, and `accessibleEntries`. Consumers repeat exactly the properties present instead of guessing a generic path kind. `accessibleEntries` preserves the sorted regular-file and directory names returned by TypeScript-Go, including directory links and junctions it followed. This distinction is semantic: `fileExists: false` and `directoryExists: true` are compatible observations of one stable directory, so a directory named `punycode.js` can fail a candidate-file probe without becoming a false cache race. A successful `readFile` carries the lowercase SHA-256 of the decoded compiler text, and a successful `realpath` carries an absolute physical path. The producer does not add an eager content read for failed resolution candidates. New consumers prefer this section for predicate-only paths; malformed or internally contradictory entries refuse reuse.
- `inputHashes` and `inputRealpaths`: optional, paired legacy projection of compiler-time graph proof retained for older consumers and for strict realized-file validation. A lowercase SHA-256 of the text returned by the compiler filesystem plus an absolute physical path records an existing file; this follows TypeScript-Go's UTF-8/UTF-16 BOM decoding rather than hashing the encoded file bytes. The shared directory-kind digest records an observed directory, and paired `null` values record the legacy collapsed missing state. A current consumer uses `inputObservations` to preserve predicate meaning, while envelopes without that field keep the conservative legacy behavior. Persistent hosts still compare realized files by exact content and physical identity before authorizing reuse. Candidate-only paths use their recorded predicate constraints because the compiler may never read them; their transition into a selectable file invalidates the generation. An executable sidecar that omits all proof sections remains watch-correct: persistent adapters retain their pre/post snapshot for ordinary in-project members and conservatively decline narrow reuse when the graph reaches outside the project walk.
- `useCaseSensitiveFileNames`: whether the compiler compared file names case-sensitively, the policy it matched the project's root specs with. TypeScript-Go decides it from the filesystem its executable lives on, not from the platform, so `@ttsc/unplugin` decides project membership under this answer rather than guessing it; an envelope without the field leaves the adapter on the platform's ordinary answer.
- `inputProofFailures`: optional machine-readable reasons why a graph member has no trustworthy proof. `driver.NewTransformGraph` emits bounded stable codes such as `accessible-entries-changed`, `content-changed`, `content-unavailable`, `file-exists-changed`, `directory-exists-changed`, `stat-changed`, `predicate-conflict`, `realpath-changed`, `resolution-changed`, or `unobserved`. An observed predicate or replayed resolution that changes refuses the generation. Consumers still treat a required absent proof as a refusal, but can identify the producer condition and exact path instead of reducing every case to "proof missing." This field is diagnostic evidence only and never authorizes reuse.

For each transformed file, `@ttsc/unplugin` also registers the `candidates` of every importer in its `edges` reachability closure and every universal `resolutionInputs` member. These inputs remain host-owned even when a plugin declares complete dependencies because a plugin cannot vouch for a compiler-resolution or automatic-type change that occurs without any plugin input changing.

`@ttsc/unplugin` registers, per transformed file `F`, the union of `reach(edges, F)`, globals, configs, reachable importer candidates, universal resolution inputs, and `dependencies[F]` as watch files. A plugin's `dependencies` can only widen the host-owned bound unless the same envelope declares that file's list [complete](#dependency-completeness). This is what makes webpack filesystem caches, including what Next.js persists under `.next/cache`, watch graphs, and Turbopack `fileDependencies` invalidate soundly by default when a type-only or resolver input changes. Malformed sections degrade to fewer registrations, never a failed build.
...
| Variable | Meaning |
| --- | --- |
| `TTSC_TSGO_ARGS` | JSON array of the tsgo CLI flags the user forwarded (`--strict`, `--declaration`, `--target es2020`, …) plus any output containment the launcher itself needs. `driver.LoadProgram` reads it whenever the caller passed no explicit `LoadProgramOptions.TsgoArgs`, so a plugin built on the driver honors forwarded compiler flags without parsing anything. A plugin that builds its own `CompilerOptions` should decode the array and merge it last, so the user's flag wins over the tsconfig: parse it with `tsoptions.ParseCommandLine` and pass both its `CompilerOptions()` and `tsoptions.CommandLineRawOptions` of it to `tsoptions.GetParsedCommandLineOfConfigFile`, as TypeScript-Go's own command line does, or a forwarded reset such as `--declarationDir null` leaves the config's value in place. |
