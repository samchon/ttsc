...
This page is the contract between `ttsc` and a plugin package. It is one page on purpose — `composes`, `contributors`, factory generics, and the wire JSON are all validated in one pipeline, and splitting them would force forward references between two co-canonical files. Most readers can start with the **Minimum contract** section (descriptor, the five subcommands, `--plugins-json`, exit codes); skip composes / contributors / factory generics on the first read.
...
Before the host invokes any transform plugin, `assertSharedHostCompatibility` (`packages/ttsc/src/compiler/internal/sharedHostHelpers.ts`) checks the resolved plugin binaries. If more than one distinct binary is active and they are not all first-party utility plugins (`@ttsc/banner`, `@ttsc/paths`, `@ttsc/strip`), the host aborts with one of:
...
The first fires during `ttsc build` / `ttsc check`; the second during `ttsc transform`. To combine third-party transform libraries from different vendors (`typia` + `nestia`-style), pick one aggregate plugin and list the others in its `composes: [...]` array. See [Reference → Pitfalls](/docs/development/reference/pitfalls#combining-plugins) for the full failure catalogue.
...
- **First-party utility names refused.** `@ttsc/banner`, `@ttsc/paths`, and `@ttsc/strip` cannot appear in `composes`. They are composed automatically through the shared compiler host hosted by `packages/ttsc/utility/host.go`.
...
A check-stage plugin that does not implement `fix` should exit non-zero with a human-readable stderr message; the host prints stderr verbatim and aborts the pipeline. `ttsc` does **not** parse any specific phrase. The `@ttsc/lint` plugin uses the convention `<plugin-name>: fix not supported` and exits `2`, but this is a project convention, not a host contract — any non-zero exit suffices. Plugins that support fixes but find nothing to apply exit `0` with empty stderr.
...
A check-stage plugin that does not implement `format` should exit non-zero with a human-readable stderr message; the host does not parse any specific phrase. The `@ttsc/lint` convention of `<plugin-name>: format not supported` and exit `2` is a project convention only. Plugins that support format but find nothing to apply exit `0` with empty stderr.
...
For leaf-text mutations (string-literal, identifier, numeric-literal `.Text`), the printer reads original source text via `getTextOfNode` when the node is non-synthesized and has a parent — see [AST & Checker → Mutating leaf text](/docs/development/concepts/tsgo#mutating-leaf-text-synthesize--reset-loc) for the synthesize-flag invariant the rewrite must follow.
