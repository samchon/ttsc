...
```json
{
  ...
  "graph": {
    ...
    "inputObservations": { "src/generated.ts": { "fileExists": false } },
...
- `inputProofFailures`: optional machine-readable reasons why a graph member has no trustworthy proof. `driver.NewTransformGraph` emits bounded stable codes such as `content-changed`, `content-unavailable`, `file-exists-changed`, `directory-exists-changed`, `stat-changed`, `predicate-conflict`, `realpath-changed`, or `unobserved`. A wholly unobserved speculative candidate has no failed compiler call and is omitted, but an observed candidate whose predicate changed is reported and refuses the generation. Consumers still treat a required absent proof as a refusal, but can identify the producer condition and exact path instead of reducing every case to "proof missing." This field is diagnostic evidence only and never authorizes reuse.
