...
All entries are normalized absolute local filesystem patterns. `root` must resolve to the same physical filesystem identity as `--cwd`; consumers reject a snapshot rooted in a different project instead of redirecting a long-lived watcher. `files` are retained while missing; `globs` describe populations and are retained while matching nothing. Windows drive, UNC, and their extended-length file forms are local paths, while device namespaces and HTTP(S) resources are outside this protocol. The command declares topology without loading a TypeScript Program or reading the external content. Hosts call it only when the capability is explicitly enabled, so older strict sidecars are never probed with an unknown command. A failing refresh leaves the consumer's last successful snapshot in force.
