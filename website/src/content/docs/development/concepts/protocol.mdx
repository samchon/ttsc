...
The CLI watcher keeps ancestor ownership so deleting, renaming, or atomically replacing a declared directory cannot strand a watcher on the old filesystem object. Glob populations use recursive `fs.watch` handles on every supported platform; a rename invalidates the affected handles before the topology is reconciled, so a replacement at the same path binds a fresh handle to the new filesystem object. Events compare the declared population and its content fingerprints before scheduling a debounced rebuild, so an unrelated file or a filename-less event with unchanged content stays quiet.
