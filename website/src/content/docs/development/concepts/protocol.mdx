...
- `candidates`: per importer, missing paths that precede its selected module target in TypeScript-Go's resolution order. Creating one changes the meaning of an unchanged import, so consumers watch and hash it. Paths below the selected target are omitted and do not invalidate a cache.
