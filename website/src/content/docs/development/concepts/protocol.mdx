...
Keep executable descriptor setup synchronous and lightweight. When a `.ts` descriptor needs the `ttsx` fallback, `ttsc` caps captured process output and descriptor JSON at 16 MiB. It sets no deadline: a descriptor that takes a long time is a slow build you can watch and interrupt, and killing it mid-evaluation would replace that with a failed build you cannot act on. An output overflow, launch failure, signal, non-zero exit, missing result, or invalid JSON stops CLI, API, and LSP setup with a cause-specific error; move long-running work into the native plugin command instead.
