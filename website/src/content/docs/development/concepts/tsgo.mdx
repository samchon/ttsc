...
> This page covers the *implementation* — how to bootstrap a Program, walk the AST, call the Checker, and emit diagnostics. If you haven't yet decided whether you need the Checker at all, see [Concepts → Do I need the Checker?](/docs/development/concepts#do-i-need-the-checker) first.

For a small source-AST transform, start with [Getting Started](/docs/development/authoring/getting-started). For the four shipped examples, read [Reference plugins](/docs/development/reference/reference-plugins). For the curated Go-side surface, see [Reference → Driver API](/docs/development/reference/driver-api).
...
- [`packages/ttsc/utility/host.go`](https://github.com/samchon/ttsc/tree/master/packages/ttsc/utility/host.go) — the first-party transform host (`loadUtilityProgram`).
- [`packages/lint/linthost/host.go`](https://github.com/samchon/ttsc/tree/master/packages/lint/linthost/host.go) — production diagnostics engine.
...
- `node.Symbol()`: bound symbol for declarations — **nil for non-declaration nodes**, see [Pitfalls](/docs/development/reference/pitfalls#ast-and-checker).
- `node.ForEachChild(fn)`: visit child nodes.
- `node.Text()`: source text for identifier/literal-like kinds. **Panics on unhandled kinds** — branch on `node.Kind` first.
...
`Node.Pos()` is the position *before* leading trivia; `SkipTrivia` walks past whitespace and comment trivia and returns the first significant token's offset. `GetTokenPosOfNode` is also valid but no production code in this repo uses it for diagnostic ranges — prefer `SkipTrivia` for consistency.
...
Reference: [`packages/ttsc/utility/host.go`](https://github.com/samchon/ttsc/tree/master/packages/ttsc/utility/host.go) for `stripRewriter`, `callExpressionName`, and statement filtering.
...
Reference: [`packages/ttsc/utility/host.go`](https://github.com/samchon/ttsc/tree/master/packages/ttsc/utility/host.go) for `pathsRewriter`, module-specifier walking, and source-to-output path mapping.
...
Source: [`packages/lint/linthost/rules_promise.go::awaitableType`](https://github.com/samchon/ttsc/tree/master/packages/lint/linthost/rules_promise.go). The example exercises three Checker categories (`GetTypeAtLocation`, `GetPropertyOfType`, `GetSignaturesOfType`) on real production code.

## Mutating leaf text: synthesize + reset Loc

For structural mutations — filtering `Statements.Nodes`, swapping children — `shimprinter.EmitSourceFile` round-trips cleanly. The printer iterates statement lists directly without reading source spans.

**For leaf-text mutations** (identifier, string-literal, or numeric-literal `.Text`), the printer reads original source text through `getTextOfNode` when the node is non-synthesized and has a parent. That means a naive `lit.AsStringLiteral().Text = "x"` silently vanishes: the printer reads the *old* span via `scanner.GetSourceTextOfNodeFromSourceFile` and ignores your `.Text`.

```go
// Before mutating identifier, string-literal, or numeric-literal text,
// detach the node from its parse-tree source span. Otherwise the printer
// reads the OLD source via getTextOfNode and the rewrite vanishes.
lit.AsStringLiteral().Text = rewritten
lit.Flags |= shimast.NodeFlagsSynthesized
lit.Loc   = shimcore.UndefinedTextRange()
```

Source: [`packages/ttsc/utility/host.go`](https://github.com/samchon/ttsc/tree/master/packages/ttsc/utility/host.go) for the `@ttsc/paths` rewriter (leaf-text pattern) and the `@ttsc/strip` `filterStatements` body (structural pattern). See [Recipes → Mutating leaf-text nodes](/docs/development/authoring/recipes#mutating-leaf-text-nodes) for the recipe form.
...
`driver.NewLintDiagnostic` wraps `(pos, end)` into a `Diagnostic` with the file's path. `driver.CountErrors` counts `SeverityError` entries. `driver.WritePrettyDiagnostics` renders to any `io.Writer` using the same colorful format `ttsc` itself uses. See [Reference → Driver API → Diagnostics](/docs/development/reference/driver-api#diagnostics).
...
- Reporting diagnostics at statement start instead of token start (use `SkipTrivia`).
- Trying to use Checker data before acquiring and deferring the release callback (`defer prog.Close()`).
- Calling `node.Text()` without a `node.Kind` guard — panics on unhandled kinds.
- Dereferencing `node.Symbol()` without a nil check — non-declaration nodes return nil.
- Mutating a leaf-text node's `.Text` without stamping `NodeFlagsSynthesized` and resetting `Loc`.
- Listing the same `Kind` twice in `Visits()` — silently deduped.

For the full failure catalogue with error strings, see [Reference → Pitfalls → AST and Checker](/docs/development/reference/pitfalls#ast-and-checker).
...
2. [`packages/ttsc/utility/host.go`](https://github.com/samchon/ttsc/tree/master/packages/ttsc/utility/host.go) — the first-party banner/strip/paths transform host (where the real logic lives; the three sidecars are 36-line dispatchers).
3. [`packages/ttsc/test/utility`](https://github.com/samchon/ttsc/tree/master/packages/ttsc/test/utility/) — utility transform behavior tests.
