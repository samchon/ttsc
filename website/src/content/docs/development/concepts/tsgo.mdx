...
`@ttsc/lint` is the exception: [`packages/lint/linthost/host.go`](https://github.com/samchon/ttsc/tree/master/packages/lint/linthost/host.go) inlines the same Program/Checker bootstrap so its standalone native engine and WASM build can share one package without importing the public `driver` facade.
