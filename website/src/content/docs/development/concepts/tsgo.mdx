...
For a small source-AST transform, start with [Getting Started](/docs/development/authoring/getting-started). For the four shipped examples, read [Reference Plugins](/docs/development/reference/reference-plugins).
...
`v0.0.0` is intentional. The modules are supplied by `ttsc`'s generated `go.work` overlay at build time. For local editor support, create your own `go.work`; see [Local Development](/docs/development/authoring/local-dev).
