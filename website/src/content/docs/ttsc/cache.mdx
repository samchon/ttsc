...
This has a consequence worth stating before any of the CI recipes below: **the key you give `actions/cache` affects your hit rate, never your correctness.** A restored cache that does not match misses on the internal key and rebuilds. A cache keyed too narrowly costs you a rebuild you did not need; one keyed too broadly costs you a rebuild you did not avoid. Neither can hand you a binary built from different inputs than the ones you are building with.
...
`pnpm` and `yarn` reconcile `node_modules` in place instead of deleting it, so the ordering is less load-bearing there. Placing the cache step after the install works under every package manager, which makes it the shape worth memorizing.
...
Then cache `.ttsc-cache`. One path covers both the plugin binaries and the Go object cache. Point it at a directory dedicated to `ttsc`: `ttsc` owns everything under that root, and `ttsc clean` removes its `plugins/` and `go-build/` there, so a shared location such as `~/.cache` is the wrong target.
...
`npx ttsc cache paths --json` reports a `cacheableRoots` array under either layout, which is the minimal set of directories a CI cache has to persist. Use it instead of hardcoding paths.
...
On the second run of an unchanged workflow that line must be absent. If it appears every time, the cache is restoring nothing useful: check that the cache step runs after the install, that the key includes `runner.arch`, and that the path matches what `npx ttsc cache paths --json` reports.
