...
`TTSC_GO_CACHE_DIR` redirects only the Go object cache. `TTSC_CACHE_DIR` redirects the compiled plugin cache and, unless `TTSC_GO_CACHE_DIR` or `GOCACHE` overrides it, the Go object cache under `go-build/` as well.
