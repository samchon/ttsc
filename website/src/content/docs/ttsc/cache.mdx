...
This page is for anyone sharing a cache across Git worktrees, wiring `ttsc` into CI, or building a container image. It covers what the build cache holds, what invalidates it, and how to persist it correctly.
...
The same root also keeps the answers of plugin descriptor evaluations under `<root>/descriptors`. A CommonJS descriptor runs in a separate runtime process on every launch, which costs seconds on Windows, so its answer is kept together with the content and physical path of every module it loaded and every candidate it probed. A later launch reuses the answer only when the descriptor, its factory context, the complete environment, the runtime executable, and the `ttsc` version all match and every one of those inputs is still in the state it was read in. An evaluation that printed anything is not kept, because a reused answer prints nothing.
...
Set `TTSC_CACHE_DIR` to the same absolute directory in every worktree. A relative value resolves from each worktree's project root and creates a separate cache there.

```bash
export TTSC_CACHE_DIR="$HOME/.cache/my-project/ttsc"
```

Use a directory dedicated to ttsc whose basename is `ttsc`. That basename lets `ttsc clean` remove both `plugins/` and the nested `go-build/`.

The compiled plugin cache is content-addressed, so worktrees with identical build inputs reuse one binary, while a branch that changes an input receives another entry. Builds of the same key are coordinated across processes.

`TTSC_GO_CACHE_DIR` redirects only the Go object cache. `TTSC_CACHE_DIR` redirects the compiled plugin cache, temporary ttsx output, and, unless `TTSC_GO_CACHE_DIR` or `GOCACHE` overrides it, the Go object cache under `go-build/` as well.

An explicit `TTSC_CACHE_DIR` is caller-owned and is not pruned automatically. Run `ttsc clean` while no worktree is building, or manage retention separately.

Run `npx ttsc cache paths --json` in each worktree to confirm that they resolve to the same root.

## GitHub Actions

The default cache root is `node_modules/.cache/ttsc`, and you can persist it directly. Source-plugin binaries and Go objects use its `plugins` and `go-build` directories. Temporary ttsx output uses `ttsx` below the same resolved workspace root, but each runtime generation is removed on exit and must not be persisted. The ordering is the part that matters.
...
Then cache `.ttsc-cache/ttsc/plugins` as the required artifact. Add `.ttsc-cache/ttsc/go-build` only as the optional accelerator. The same variable places temporary ttsx output under `.ttsc-cache/ttsc/ttsx`, so a nested tsconfig cannot create a second project-local cache root. Point the root at a directory dedicated to `ttsc` whose basename is `ttsc`: `ttsc clean` always removes its `plugins/`, and that basename is the safety proof that lets it remove the nested `go-build/` too. A shared location such as `~/.cache` is therefore the wrong target. Keep the root, and `TTSC_GO_CACHE_DIR`, outside every plugin's Go sources as well: a plugin binary is keyed on its sources, so a build refuses a cache inside them instead of changing the sources it was keyed on while it builds. A project whose root is itself a plugin's Go module therefore needs a root elsewhere, or below its `node_modules`, which the sources never include.
