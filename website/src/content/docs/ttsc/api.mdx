...
The constructor takes the same [`ITtscCompilerContext`](#context-options) and launches the host synchronously; the project compiles lazily, so the first `transformFile` resolves once that compile lands.
