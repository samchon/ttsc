...
A project that sets `jsx` to `preserve` or `react-native` keeps JSX in its output for another tool, as Next.js projects do by default. Node cannot parse JSX, so `ttsx` compiles it in its private runtime build with the factory the project declares:

- A `jsxFactory`, `jsxFragmentFactory`, or `reactNamespace` selects the classic transform (`react`). If the project also sets `jsxImportSource`, which `preserve` allows but the classic transform does not, the declared factory wins and the import source is set aside for the runtime build.
- Otherwise `ttsx` uses the automatic runtime (`react-jsx`). It reads `jsxImportSource` (default `react`) and per-file `@jsxImportSource` pragmas. A file that relies on a per-file `@jsx` factory pragma needs `@jsxRuntime classic` beside it, as it would under `react-jsx` anywhere.
