...
So `npx ttsx clear.ts` works even though `npx ttsc` ignores that file. The entry is compiled through a project that inherits every option from the discovered `tsconfig.json` — `paths`, `strict`, `target`, `types`, and the module format — and declares only that entry, so your `outDir` never gains a file. The entry is still type-checked: a type error in `clear.ts` stops the run exactly as one in `src` would.
