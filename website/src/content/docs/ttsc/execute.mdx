...
A project that declares no `rootDir` needs none. `ttsx` compiles into a cache directory of its own and pins the source root the compiler would otherwise infer — the directory holding your `tsconfig.json` — so a check-only project with `noEmit: true` and its sources under `src/` runs unchanged, with no `.js` left beside them. Positional `ttsc <file.ts>` resolves it the same way. A `rootDir` you do declare is always used as written.
