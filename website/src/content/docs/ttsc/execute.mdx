...
`ttsx` installs one synchronous Node module hook — a `resolve` hook that covers both `import` and `require` — which redirects every raw `.ts` it resolves to the JavaScript tsgo emits for it. A dependency that ships raw TypeScript is compiled by tsgo through its own `tsconfig` (or a synthesized one when it ships none) — a full, type-checked build with your plugins applied — into a per-package cache, and the hook resolves the dependency's `.ts` sources to that emit. Node never sees raw `.ts`, so the strip-only limits never apply, and none of this touches the dependency's `exports`, sources, or `tsconfig`.

The hook never weakens the compile gate: the up-front tsgo build still deep-checks the whole program, imported workspace neighbours included. The dependency build itself runs once and is reused across runs via a freshness stamp (the ttsc version, the resolved compiler options, the tsconfig, and the source mtimes); a dependency's own type error fails the run, and a genuinely missing module still throws `ERR_MODULE_NOT_FOUND`. This makes `ttsx` strictly stronger than `tsx`: a full type-check plus the same runtime reach.
