...
A diagnostic never withholds an emit-only build's JavaScript, so a `noEmitOnError` in that project's config is switched off for it.

A root, and an entry outside `include`, is compiled with the project's own `tsconfig.json`, read where it lives, with only the file list replaced by that one file. `${configDir}`, `types`, and the default `typeRoots` resolve exactly as they do for the project, and nothing is written beside the config, so a read-only checkout, mount, or volume and an installed package's directory work unchanged. The compiler's command line cannot combine a project with a file list, so this build runs through the `ttsc` binary of the platform package (`@ttsc/<os>-<arch>`), which compiles with the TypeScript-Go version ttsc is built on rather than your project's `typescript`. A plugin's native host receives the file the same way, through `TTSC_ROOT_FILES`.
