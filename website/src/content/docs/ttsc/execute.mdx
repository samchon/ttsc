...
If a script writes new `.ts` files inside its own project and then loads them, `ttsx` compiles that generated source directory on demand with the same tsconfig and transform plugins. The generated-source emit is content-keyed, so child processes can share it without reusing stale output when a later generated file appears.
