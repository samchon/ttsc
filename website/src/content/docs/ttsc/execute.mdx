...
- A `jsxImportSource` selects the automatic runtime (`react-jsx`), even when the project also declares a classic factory. `preserve` allows both, and the type-checker then reads JSX through the import source, so the factories are set aside for the runtime build.
