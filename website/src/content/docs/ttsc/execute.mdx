...
A project that sets `jsx` to `preserve` or `react-native` keeps JSX in its output for another tool, as Next.js projects do by default. Node cannot parse JSX, so `ttsx` compiles it in its private runtime build with the factory the project declares. A `jsxFactory` or `jsxFragmentFactory` selects the classic transform (`react`). Otherwise `ttsx` uses the automatic runtime (`react-jsx`), which reads `jsxImportSource` (default `react`) and per-file `@jsxImportSource` pragmas. The same rule applies to a `--jsx` value forwarded before the entry, to dependencies built through their own `tsconfig.json`, and to `ttsc/register`. A `.tsx` file with no project compiles with the automatic runtime. The `react`, `react-jsx`, and `react-jsxdev` modes already produce executable JavaScript and are used as configured. Your `tsconfig.json` and ordinary `ttsc` output are unchanged.
