...
Any flag `ttsc` does not own is forwarded as-is to `tsgo`, so classic `tsc` compilerOptions work on the command line too, `ttsc --strict src/main.ts`, `ttsc --target es2020`, `ttsc --listFiles`. The usual home for `target`, `module`, `strict`, `lib`, and friends is still `tsconfig.json`; the CLI form just overrides it for one run. On a project that configures a ttsc plugin, the forwarded flag still reaches the in-process compiler, though tsgo's reporting flags (`--listFiles`, `--showConfig`, …) only print on a plain, plugin-free build.
