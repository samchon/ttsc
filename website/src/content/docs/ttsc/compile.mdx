...
| `ttsc --init` | Write a starter `tsconfig.json`. Works in a directory that has no project yet, so it is the first command in a fresh package. |
...
Flag names are matched the way `tsc` matches them: case-insensitively, with one or two leading dashes. `--noEmit`, `--noemit`, and `-noEmit` all select the same flag, so a CI script or a Windows shell that spells a flag differently gets the behaviour it asked for instead of a silently forwarded flag `ttsc` did not recognize.
