...
The watcher follows TypeScript-Go's resolved program inputs instead of guessing from directory names. It reconciles the set after each build and when directory topology changes, so newly included folders, `extends` configs, project references, loaded declarations, and selected Go plugin sources join the running session. A Go plugin is watched as its build reads it: the whole Go module its `source` belongs to, so an edit to a sibling package or to the module's `go.mod` rebuilds the session, along with every contributor's source, while a write below the module's `node_modules`, `.git`, or `.ttsc` does not. A check-stage plugin may also publish exact local files and glob populations through the project-input protocol; missing exact paths and zero-match globs remain live, while unrelated workspace documents stay quiet. Executable plugin-selection dependencies additionally declare exact reload files and immediate directory topology, causing a fresh launcher evaluation when module resolution or contributor selection changes. Compiler output and cache products are not inputs, so an emit-mode build stays quiet until a real input changes.
