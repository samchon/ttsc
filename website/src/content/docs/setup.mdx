...
Start the agent from your project root, or pass `--cwd` and `--tsconfig`. Usage guidance is delivered in the MCP `initialize` response; the server never writes into your `CLAUDE.md` or `AGENTS.md`. On codegraph's own agent-cost benchmark it cuts an agent's tokens by 74% and tool calls by 88%, averaged across two repositories, with the agent reading zero files; see [Code Graph](/docs/graph) and its [Benchmark](/docs/graph/benchmark).
