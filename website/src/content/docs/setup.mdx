...
- **Run inside a bundler**: Vite, Webpack, Rollup, and friends, see below.
...
When a bundler owns the build, the `ttsc` CLI never runs, so its plugin pass would be skipped. Install [`@ttsc/unplugin`](/docs/ttsc/bundler) to run the same pass from inside the bundler:
