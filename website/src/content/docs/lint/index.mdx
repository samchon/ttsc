...
Declaration files and JavaScript stay selection-driven. A `.d.ts` is typings rather than authored source, and JavaScript enters the Program only through `allowJs`, so both are linted when your own `tsconfig.json` selects them and not because an import reached them.

Writes stay inside the project. `ttsc fix` and `ttsc format` rewrite only the files your `tsconfig.json` selected, so importing a sibling package never reformats it. A diagnostic on an imported source is still reported, and the package that owns the file fixes it from its own run.
