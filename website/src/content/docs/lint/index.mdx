...
Rules contributed by another Go package use the same collision-free band. Their codes are deterministic for an unchanged complete contributor set and do not depend on registration order. Changing that contributor set can change contributor assignments when preferred codes collide, but it cannot change a built-in assignment.
