...
- **470+ lint rules**, grouped by family (Core, TypeScript, React, JSX-A11y, Promise, Solid, Testing Library, Jest, Vitest, Playwright, Cypress, Storybook, Next.js, TanStack Query, Regexp, Security, JSDoc, Functional, Boundaries, eslint-comments, react-perf). Severity per rule (`error` / `warning` / `off`). Replaces `eslint`.
