...
`format/print-width` and `format/trailing-comma` overlap in one place: the broken layout that `format/print-width` emits always has a trailing comma (structurally, not because of the rule). So leaving `format/trailing-comma` at `"off"` doesn't strip trailing commas from reflowed multi-line literals, only from manually-written ones.
