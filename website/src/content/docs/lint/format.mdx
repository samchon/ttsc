...
Both `sortImports` and `jsDoc` are **opt-in**: they only run when you set their `format` keys. Every other format behavior runs as soon as a `format` block is present, including the keyless layout passes (statement splitting, indentation, whitespace, clause joining, declaration headers, ternary and nullish parens, orphan semicolons, and parameter properties).
