...
Inner spaces in single-line braces. `true` (default) keeps the space inside object literals, destructuring patterns, named imports/exports, type literals, mapped types, and import attributes (`{ a }`); `false` removes it (`{a}`). Block, class, interface, and enum braces are unaffected. Driven by `format.bracketSpacing`.
...
Width is measured in **display columns** (Prettier's `getStringWidth`), not UTF-8 bytes. Text is segmented into Unicode extended grapheme clusters; an emoji cluster costs two columns whole, however many code points it holds, and every other cluster is charged by the East_Asian_Width of its base character. So a Hangul or CJK identifier is charged its true width, `👨‍👩‍👧` and `👋🏽` cost two columns rather than eight and four, and a combining mark costs nothing. The property data is the Unicode 16.0.0 database, not an approximation of it. One deviation is deliberate: a tab counts as one column here, where Prettier counts zero.
