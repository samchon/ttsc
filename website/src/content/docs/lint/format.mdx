...
Keyless. Normalizes the leading indentation of each statement's first line to its block-nesting depth (`tabWidth` columns per level, or one tab per level under `useTabs`). It touches only a statement's own starting line; interior and continuation lines belong to `format/print-width`.
