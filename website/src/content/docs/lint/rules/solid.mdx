...
`className` and `htmlFor` are autofixed: the rename is 1:1 and only the name token is rewritten, so the value — a string, an expression container, or nothing at all — survives untouched. The `key` arm stays diagnostic-only. A Solid DOM element does not consume `key`, so its resolution is a deletion rather than a rename, and the deletion has to take the surrounding whitespace with it to leave valid JSX.
