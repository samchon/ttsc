...
The finding is tagged `Deprecated`, so an editor strikes the title through: the separator is superseded, not broken, and the story still renders.
...
The finding is tagged `Unnecessary`, so an editor greys the annotation out. Both shapes the rule reports — the `name` / `storyName` property inside the story object, and a `Story.storyName = ...` assignment — are reported at exactly the range whose deletion resolves the finding.
...
The finding is tagged `Unnecessary`, so an editor greys the property out; the reported range is the property itself, and removing it is the resolution.
