...
The finding is tagged `Unnecessary`, so an editor greys the annotation out. Both shapes the rule reports are covered: the `name` / `storyName` property inside the story object and a `Story.storyName = ...` assignment. Each is reported at exactly the range whose deletion resolves the finding.
