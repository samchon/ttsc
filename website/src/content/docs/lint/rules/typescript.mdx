...
- [`typescript/no-floating-promises`](#rule-typescript-no-floating-promises): Reject Promise-typed expressions whose result is discarded, most often a bare `getPromise();` expression statement.
...
- [`typescript/no-misused-promises`](#rule-typescript-no-misused-promises): Reject Promise values supplied where a non-Promise was expected.
...
- [`typescript/no-unsafe-assignment`](#rule-typescript-no-unsafe-assignment): Reject assigning an `any`-typed value into a concretely typed location.
...
- [`typescript/prefer-as-const`](#rule-typescript-prefer-as-const): Prefer `as const` over `as "literal"` assertions.
...
- [`typescript/require-await`](#rule-typescript-require-await): Reject `async` functions whose body contains no `await` expression.
...
- [`typescript/await-thenable`](#rule-typescript-await-thenable): Reject `await` on operands that are not thenable.
- [`typescript/ban-ts-comment`](#rule-typescript-ban-ts-comment): Reject `@ts-ignore` and `@ts-expect-error` comments.
...
- [`typescript/switch-exhaustiveness-check`](#rule-typescript-switch-exhaustiveness-check): Require every member of a union or `enum` discriminant to be covered by an explicit `case`.
...
Reject `await` on operands that are not thenable, `for await...of` over values that are not async iterable, and `await using` of resources that are not async disposable.

Type-aware. The Checker decides whether the awaited expression has a callable `then` method, whether the `for await...of` source exposes `[Symbol.asyncIterator]`, and whether each `await using` initializer exposes `[Symbol.asyncDispose]`. The protocol lookups resolve through unions (any async constituent is accepted), intersections, type aliases, inherited members, and generic constraints. Autofixable: drops the `await` on a non-thenable operand. The `for await...of` and `await using` findings carry no autofix because removing that `await` changes runtime behavior.
...
```ts
// reports: typescript/no-restricted-types (error)
const a: Object = {};
```
