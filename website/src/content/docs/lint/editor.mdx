...
"Saved project state" means the project as it is on disk right now, not as it was when the language server started. A file that changed on disk while it was closed is re-read when you open it, so a branch switch, a `git pull`, or a generator run takes effect without restarting the language server. A change to what selects or builds the plugins ends the language server session, and the editor starts a new one that selects, builds, and runs the current plugins. That covers a `tsconfig.json` or `jsconfig.json` in the project's config chain, whose `compilerOptions.plugins` names them, a `package.json` whose dependencies publish plugins, a plugin itself, its descriptor, a file the descriptor reads, and its Go sources, as `ttsc check --watch` starts a fresh session for the same changes.
