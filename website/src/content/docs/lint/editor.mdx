...
"Saved project state" means the project as it is on disk right now, not as it was when the language server started. A file that changed on disk while it was closed is re-read when you open it, and a `tsconfig.json` or `jsconfig.json` edit is picked up from the editor's file watcher, so a branch switch, a `git pull`, a generator run, or a config change takes effect without restarting the language server. A change to a plugin itself, its descriptor, a file the descriptor reads, or its Go sources, which a package update or a plugin under development makes, ends the language server session, and the editor starts a new one that builds and runs the current plugin.
