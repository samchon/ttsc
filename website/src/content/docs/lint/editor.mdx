...
Completion uses the live editor buffer, so it remains available while the file is dirty. The rule's completion corpus is discovered in the background when the language server starts, and rediscovered after you save a file, change the editor configuration, or change a watched file. Enabling a rule or changing a contributor's indexed inputs therefore reaches the editor on the next save, without a restart. Discovery runs in the background, so the new items appear a moment after the save rather than during it.
