...
"Saved project state" means the project as it is on disk right now, not as it was when the language server started. A file that changed on disk while it was closed is re-read when you open it, and a `tsconfig.json` or `jsconfig.json` edit is picked up from the editor's file watcher, so a branch switch, a `git pull`, a generator run, or a config change takes effect without restarting the language server. Only an unsaved buffer is excluded, which is why a save is still what publishes your own edits.
...
The completion above is published by one rule. Most rules assist through diagnostics, suggestions, and fixes instead, and not every rule publishes completion. The [video library](/docs/lint/videos) shows two rules from each of the 21 families and what each one puts in front of you.
