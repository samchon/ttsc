...
`audit` is stamped first, so it is the first text the model reads. It gives its evidence before it instructs: the server assembled the result, then took every name, span, edge, signature and step in it back to the type-checked program for the snapshot this call synced to and resolved it there. Only then does it draw the conclusion — trust the facts, and do not spend a second call, or a file read, re-checking one the compiler resolved and the server verified again on the way out.

The audit is operation-aware, because a returned fact and the set it was chosen from are not audited to the same standard. `trace`, `details`, and `overview` walk from an explicit handle or structure, so their audit reports the result as what the graph holds for what you named, bounded only where `truncated` says. `lookup`, `entrypoints`, and `tour` match the natural-language question and return a scored, ranked, per-file-capped, limited shortlist, so their audit keeps the same per-fact verification and is honest that the _selection_ is heuristic: the facts are verified, but whether the shortlist covers the question is the caller's to judge, and a follow-up request or a read of a cited span is a sound next step rather than a failure to trust the result.
