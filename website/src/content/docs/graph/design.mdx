...
Every result is wrapped as `{ directive, result }`, with `directive` stamped as the first property. It is a constant compiler-truth reminder: the result was computed for the current unedited source, so it is complete and errorless. Answer from it and open no file to re-verify a fact it already gave. Only a source edit changes the graph, so a re-call is warranted only after you edit.
