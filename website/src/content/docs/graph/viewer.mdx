...
The graph is explorable, not just viewable. Click a node to focus it: its neighborhood lights up, the rest dims, and an info panel shows the declaration, its file, and its edges by family, with an "isolate 2 hops" button that re-lays out just that neighborhood. The explorer sidebar goes further: the Files tab is a directory tree of the graph (click a directory or file to scope the view to it), and the Symbols tab finds a declaration by name (picking a result flies the camera to it) and toggles node kinds and edge families on and off.
