...
Edges are colored by family. The graph itself distinguishes finer relationship kinds (the full set is typed in [`ITtscGraphEdge.ts`](https://github.com/samchon/ttsc/blob/master/packages/graph/src/structures/ITtscGraphEdge.ts)); the viewer folds them into four so the picture stays readable:
