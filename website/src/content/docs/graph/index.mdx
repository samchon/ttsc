...
The graph hands it the answer up front through one MCP tool, `query`. The agent chooses one request branch per call: `overview`, `entrypoints`, `lookup`, `trace`, `details`, or `escape`. The agent stops fanning out across files.
