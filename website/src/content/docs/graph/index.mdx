...
The graph hands it the answer up front through one MCP tool, `inspect_typescript_project_graph_before_answering`. The agent chooses one request branch per call: `overview`, `entrypoints`, `lookup`, `trace`, `details`, or `escape`. The agent stops fanning out across files.
...
- **What is this, exactly?** The declaration's source, its dependencies, and its dependents: `details`.
...
- [Tools](/docs/graph/tools): the single MCP tool, its request/result branches, and the node and edge kinds.
