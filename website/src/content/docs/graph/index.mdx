...
The graph hands it the answer up front. `graph_overview` maps the project's layers, hotspots, and public API with no source read; `graph_query` finds the right symbol from a natural phrase; `graph_trace` follows the flow from a route or symbol to where the work happens and back out to what a change would reach; `graph_expand` reads the source once the graph has located it. The agent stops fanning out across files.
...
- **What does this reach, and what reaches it?** Forward to callees, reverse to callers, and an impact projection onto the public API, routes, and tests — `graph_trace`.
- **Which route or component serves this?** Framework routes (NestJS controllers, Next.js pages) are synthesized onto the same graph and traced like any other edge.

A tree-sitter graph has no types and resolves edges by guessing from text. An editor's language server has types but no architecture projection. Only a graph built on the real checker holds the resolved structure, the framework routes, and the affected set in one place — and every edge says whether it is `checker-resolved`, `framework-derived`, or `heuristic`, so an agent never mistakes inference for fact.
...
- [Tools](/docs/graph/tools): what `graph_overview`, `graph_query`, `graph_trace`, and `graph_expand` return, the provenance and confidence on every relationship, and the node and edge kinds.
