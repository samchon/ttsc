...
`timeoutMs` defaults to 60 seconds and covers time queued behind an earlier boot, fetch, instantiation, and the Go readiness signal. Aborting any caller that joined the same in-flight boot cancels that shared attempt. Rejection clears its cache entry and readiness callbacks so a later call can retry.

`importScripts` is synchronous. `bootTtsc` checks cancellation immediately before and after it but cannot interrupt script evaluation. JavaScript also cannot terminate a Go runtime after `go.run` starts; if cancellation occurs while waiting for readiness, terminate that Worker before retrying.
...
| Boot times out or is aborted while waiting for readiness | Terminate the Worker before retrying. The old Go runtime cannot be interrupted after `go.run` starts and must not share a new readiness bridge. |
