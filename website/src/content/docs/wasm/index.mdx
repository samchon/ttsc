...
`@ttsc/wasm` is the in-browser runtime for `ttsc`. It packages the same Go-backed engine the CLI ships with — built for `GOOS=js GOARCH=wasm` — alongside a JS boot helper and an in-memory filesystem bridge. Use it to embed a `ttsc` playground in your own site, or to extend the wasm with your plugins.
