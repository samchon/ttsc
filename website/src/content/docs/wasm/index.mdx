...
`timeoutMs` defaults to 60 seconds and covers time queued behind an earlier boot, fetch, instantiation, and the Go readiness signal. Aborting any caller that joined the same in-flight boot cancels that shared attempt. A rejection before `go.run` clears its cache entry and readiness callbacks so a later call can retry.
