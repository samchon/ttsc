...
Install the VS Code extension to run your project's `ttscserver` / TypeScript-Go language server in VS Code, with live TypeScript-Go editor features plus saved-state ttsc plugin diagnostics and actions in the same editor stream. Install it from the VS Code Marketplace (search `ttsc`, or `ext install samchon.ttsc`), or run `npx @ttsc/vscode` to install the bundled `.vsix` from npm:
