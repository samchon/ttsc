...
On codegraph's own benchmark it cut an agent's tokens by 70% and tool calls by 83%, averaged across two repositories, with the agent reading zero files.

See [`@ttsc/graph`](https://github.com/samchon/ttsc/tree/master/packages/graph) and the [benchmark](/docs/benchmark#code-graph-mcp).
