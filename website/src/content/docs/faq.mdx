...
The TypeScript-Go preview that `ttsc` runs on is **5–10× faster** than the JavaScript `tsc` for type-checking on real projects. `@ttsc/lint` adds a marginal cost (a single AST pass shared with the type-check, so no second parse). For most repos, `ttsc --noEmit` finishes in a fraction of the time the old `tsc + eslint + prettier --check` stack used.
...
No, `ttsc` works without it. But the whole point of the toolchain is the pair. Without `@ttsc/lint` you keep `eslint`+`prettier` as separate tools and lose the "single compile pass" benefit. The recommended install is `npm install -D ttsc @ttsc/lint @typescript/native-preview`.
...
Yes. `@ttsc/vscode` is published on npm with a one-shot installer. Run `npx @ttsc/vscode`; it downloads the package and installs the bundled `.vsix` into VS Code, nothing to keep as a dependency:

```bash
npx @ttsc/vscode
```

The installer calls `code --install-extension` against the `.vsix` bundled in the package. Marketplace release is tracked for v1.
...
4. Save the file before checking plugin diagnostics or running the command-palette `ttsc: Fix all lint issues` / `ttsc: Format document` commands. TypeScript-Go diagnostics are live; ttsc plugin diagnostics, code actions, and those command-palette commands use the saved project state, returning editor-applied `WorkspaceEdit`s only while touched documents are clean. Editor format-on-save is the exception, it formats the live unsaved buffer.
...
For VS Code, add packages as workspace folders when each package should keep its own active language-server context. Plugin diagnostics, code actions, and command computation currently use saved project state, so save files before relying on lint fixes or format edits; the edits themselves are returned as VS Code `WorkspaceEdit`s.
...
- [Lint & Prettier](/docs/lint): the rule catalog and `lint.config.ts` reference.
