...
Yes. It is on the VS Code Marketplace (search `ttsc`, or `ext install samchon.ttsc`), but the same build is also published on npm with a one-shot installer. Run `npx @ttsc/vscode`; it downloads the package and installs the bundled `.vsix` into VS Code, nothing to keep as a dependency:

```bash
npx @ttsc/vscode
```

The installer calls `code --install-extension` against the `.vsix` bundled in the package.
