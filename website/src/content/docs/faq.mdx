...
Yes. `@ttsc/vscode` is published on npm with a one-shot installer:
...
2. Confirm the same plugin diagnostics appear in the CLI:
   `npx ttsc --noEmit -p <selected tsconfig>`.
3. Make sure the plugin config is discoverable from the selected project root
   or set `configFile` in the tsconfig plugin entry.
4. Save the file before checking plugin diagnostics or running plugin-backed
   fix/format commands. TypeScript-Go diagnostics are live; ttsc plugin
   diagnostics, code actions, and command computation currently use the saved
   project state, and fix/format commands return editor-applied
   `WorkspaceEdit`s only while touched documents are clean.
...
For VS Code, add packages as workspace folders when each package should keep
its own active language-server context. Plugin diagnostics, code actions, and
command computation currently use saved project state, so save files before
relying on lint fixes or format edits; the edits themselves are returned as VS
Code `WorkspaceEdit`s.
