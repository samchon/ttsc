...
4. Save the file before checking plugin diagnostics or running plugin-backed fix/format commands. TypeScript-Go diagnostics are live; ttsc plugin diagnostics, code actions, and command computation currently use the saved project state, and fix/format commands return editor-applied `WorkspaceEdit`s only while touched documents are clean.
