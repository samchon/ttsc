...
For VS Code, add packages as workspace folders when each package should keep its own `ttsc`, `@typescript/native-preview`, and plugin config. Save files before relying on plugin diagnostics, lint fixes, or format edits.
