...
Most ESLint stylistic rules have direct `@ttsc/lint` equivalents (`noVar`, `preferConst`, `eqeqeq`, etc.). Plugin-shipped ESLint rules (`eslint-plugin-react-hooks`, `eslint-plugin-import`, etc.) don't yet have `@ttsc/lint` counterparts — for those, the native rule set is being built out rule by rule.
