...
Not directly. `@ttsc/lint` is a separate engine. Third-party rule packages can ship as `@ttsc/lint` contributor plugins — see the [Reference plugin tour](/docs/development/reference/reference-plugins#authoring-a-lint-rule-contributor) for the Go API authors implement.
...
On a fresh install, `ttsc` compiles each plugin binary into a cache the first time you run it. On a typical machine this is under 10 seconds per plugin and happens once per `node_modules`. Pre-warm with `npx ttsc prepare`. Drop the cache with `npx ttsc clean`.
...
## VSCode doesn't show lint diagnostics live — what's wrong?

1. Confirm `ttsc` is installed in the project: `npx ttsc --version`.
2. Open **View → Output**, pick **ttsc** from the dropdown, and read the server log.
3. Run **ttsc: Restart language server** from the command palette.
4. Set `ttsc.trace.server` to `"verbose"` in VS Code settings if the log is empty.
