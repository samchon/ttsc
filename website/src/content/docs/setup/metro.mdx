...
Metro builds each module's source map from the AST the Babel transformer returns, and that transformer parsed the `ttsc` output, not your file. The `ttsc` pass returns a map back to your text, and `@ttsc/metro` moves the AST's locations through it before Metro sees them, so stack traces, breakpoints, and the red box point at the lines you wrote. Code a plugin generated has no line of yours, so it takes the location of the code around it.
...
- **Cost model.** The transform core type-checks the whole project. Metro runs a multi-process worker pool, and the workers share each compile through the session `withTtsc` opens: the first worker to need a project state compiles it, and the others adopt it once they have proven that state themselves. A cold build and each edit therefore compile the project once, not once per worker. A resident incremental compiler is the next step, tracked in [#255](https://github.com/samchon/ttsc/issues/255).
