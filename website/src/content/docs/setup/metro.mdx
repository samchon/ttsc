...
A missing module-resolution candidate that would outrank a selected target is recorded as a missing fingerprint input, including when it lies under the project root. Creating that file changes the next Metro cache key even though the first project walk could not hash a path that did not yet exist.
