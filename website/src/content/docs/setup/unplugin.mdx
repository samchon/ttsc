...
The transform cache validates against the same input set: every regular file reached by the non-following project walk plus graph-reported inputs outside that walk. Hosts that keep one cache for the process lifetime instead of per build (Metro workers, the Turbopack loader, Bun) therefore recompile when an input such as a `node_modules` declaration, a monorepo sibling source, or a file reached through a symlink or Windows junction changes, not only on ordinary in-project edits.
