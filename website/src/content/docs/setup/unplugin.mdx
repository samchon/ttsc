...
The adapter yields to the next Bun loader for declarations, `node_modules`, and source that `ttsc` leaves unchanged, so its broad TypeScript hook does not shadow another plugin or Bun's built-in loader. `Bun.build` clears the project generation through its `onStart` lifecycle on every build.
