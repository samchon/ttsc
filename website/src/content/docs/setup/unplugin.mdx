...
Explicit `include` and `files` specifications limit root-file discovery, including inherited paths and `${configDir}` substitutions. A `files`-only project has no implicit include. The adapter skips unrelated directory trees and their rename events, so test scratch directories outside the configured search cannot invalidate a transform generation. Matching new files and directories still invalidate it, including when the project is opened through a junction or a Windows short path. Case-insensitive matching includes Unicode case variants. Imports and plugin dependencies outside that search remain tracked as external inputs; `include` is not an import restriction.
