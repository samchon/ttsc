...
Membership comes from the resolved configuration rather than from a list of directory names. `allowJs` and `resolveJsonModule` decide which extensions can enter the program, `outDir` and `declarationDir` and the plain entries of `exclude` name the directories it does not contain, and only `.git`, `node_modules` and `.ttsc` remain excluded by name, because no tsconfig can name them. Adapter `compilerOptions` replace inherited `outDir` and `declarationDir` values before membership is derived, while explicit top-level `exclude` entries remain in force.
