...
The digest records the entries the walk considers rather than each directory's own stamp, and a directory takes part only when its subtree can hold a program input. A bundler's output directory therefore costs nothing whatever it is called and however its filenames churn, while the directory is still walked and still watched so a source appearing in it later is detected. Validation hashes only the generation's declared inputs, which is exactly the set it compares over, so a tree of emitted files costs no reads.
