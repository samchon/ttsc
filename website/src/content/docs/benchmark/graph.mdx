...
These tabs read the actual package source. The benchmark claims above should be judged against these files, not copied type snippets.
...
<Tabs
  items={[
    ...
    <code key="instructions">instructions.ts</code>,
    <code key="result-guide">resultGuide.ts</code>,
  ]}
>
  <Tabs.Tab>
    <LocalSource path="packages/graph/src/structures/ITtscGraphApplication.ts" showLineNumbers />
  </Tabs.Tab>
  <Tabs.Tab>
    <LocalSource path="packages/graph/src/structures/ITtscGraphTour.ts" showLineNumbers />
  </Tabs.Tab>
  <Tabs.Tab>
    <LocalSource path="packages/graph/src/structures/ITtscGraphNext.ts" showLineNumbers />
  </Tabs.Tab>
  <Tabs.Tab>
    <LocalSource path="packages/graph/src/server/instructions.ts" showLineNumbers />
  </Tabs.Tab>
  <Tabs.Tab>
    <LocalSource path="packages/graph/src/server/resultGuide.ts" showLineNumbers />
  </Tabs.Tab>
</Tabs>

## How The Comparators Differ

The benchmark harness treats `codegraph` and `codebase-memory-mcp` as separate comparator arms, not as the same contract.
