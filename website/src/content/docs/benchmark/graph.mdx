...
node experimental/benchmark/graph.mjs --project=typeorm --models=sonnet,opus,codex --tools=ttsc-graph,codegraph

# Full sweep: runs projects sequentially because the agent cells are expensive
node experimental/benchmark/graph.mjs --all --models=sonnet,opus,codex --tools=ttsc-graph,codegraph
```
