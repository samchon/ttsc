...
The current refresh starts from a clean agent-cost grid. It records only **Codex / GPT-5.4 mini** empty-MCP baseline cells at N=5. `@ttsc/graph` and `codegraph` comparator measurements are intentionally absent until those arms are run, so the dashboard shows baseline rows without savings bars for this slice.
