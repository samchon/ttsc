...
> ## TL;DR
>
> I installed codegraph and codebase-memory-mcp. They advertise cutting an agent's tokens by tens of times — cbm headlines **120×**. In practice my token bill didn't move, and Claude Code actually got *worse* at the task.
>
...
Same question, same agent. Only `@ttsc/graph` stays flat at ~42k tokens across every repo. The others swing wildly — and some even spend *more* than the baseline (no MCP at all). Full numbers and method are on the [benchmark page](https://ttsc.dev/docs/benchmark/graph).
...
And their core claim is legit. The enemy is the agent's **grep · find · Read crawl loop.** Every "where is this?" turns into a grep, a file open, an import chased, another grep — dozens of round-trips burning tokens and time. codegraph's pitch is clean: replace that loop with one graph query — "58% fewer tool calls, file reads to ~zero." cbm goes further, headlining "**120× fewer tokens**" (a 99.2% cut).

Sounded great. So I installed them.

One thing up front, though. Killing grep-spam is the **easy** win. The hard part is killing it *while also* cutting tokens. Those two are not the same thing.

### 2.2. So I Tried Them

Here's what actually happened.

- **Tokens didn't drop at all.** The tool-call *count* may have fallen, but the thing you pay for — tokens — didn't budge. On some cells it was even *worse than the baseline* (codegraph spent up to 47% more, cbm up to 96% more).
...
- I couldn't just ask in plain language. Every query had to be hand-shaped — cbm wants a Cypher query with an exact `qualified_name`; codegraph wants you to name the symbols for a flow. That **babysitting** alone ate the productivity.

In short: worse than before I installed them. The opposite of the ad. So I went digging.

### 2.3. Bodies and Pointers

The first cause was *what* each tool hands back.
...
Easy to get confused here. "cbm's pointers are compact too — isn't that the same as your index?" No. **A pointer gives you "where" (a coordinate), not "what it does and how it connects."** And the answer to "how does this work?" is *relationships*, not coordinates. So an agent handed only coordinates has to open the files and read the relationships itself. Compactness isn't the issue — whether the compact thing is *an answer* or *a stepping stone to source* is. cbm's pointer is a stepping stone, so you cross the river anyway. (Why `@ttsc/graph`'s index is the answer itself: §3.3.)

codegraph via bodies, cbm via pointers-then-reads — that's why neither moves the token bill.
...
codegraph spells it out: "use it instead of reading files," "call it before you Read," "don't grep or Read first," "almost any question → explore." cbm too: "use instead of grep," "first call search_graph, then…," plus auto-indexing on session start.
...
Why force it? Because their headline numbers — "58% fewer tool calls," "file reads → 0" — are hostage to the call rate. The agent has to route everything through the tool for those numbers to show up. codegraph even admits it in its README: *"it only helps when queried directly; otherwise it's overhead."* So, to avoid being overhead, it forces.
...
Second, Claude Code won't always pick the **right tool at the right moment.** The more options, the more misfires — cbm exposes **14** MCP tools. (codegraph avoids that trap with a single default tool; it just forces that one tool too hard — the §3.1 problem.)
...
As you can see, `question → draft → review` are **forced function arguments.** typia compiles this type into the tool's JSON schema and validator, so an unfilled CoT is rejected at the call boundary. In typia's words: *"free prose can hide a skipped step; a typed submission cannot."* (Deeper: [CoT compliance](https://typia.io/blog/function-calling-harness-2-cot-compliance/), [function-calling harness](https://typia.io/blog/function-calling-harness-qwen-meetup-korea/).)
...
And the thesis I opened in §2.1 closes here. `@ttsc/graph` kills the grep crawl loop too — that's table stakes. The difference is it does so *without* the tokens blowing up.
