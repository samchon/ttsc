...
> ## TL;DR
>
> codegraph and codebase-memory-mcp had the idea first: hand a coding agent a code graph over MCP, so it stops crawling files one by one. Good idea. But on my own open-ended questions the token bill didn't budge, and driving the tools was a chore. So I learned from where they hit walls and built one aimed at that case.
>
> [`@ttsc/graph`](https://github.com/samchon/ttsc/tree/master/packages/graph) reads the graph the TypeScript compiler already resolved and hands the agent an **index** — never source bodies. One tool, plus a forced chain-of-thought, keeps it from wandering off.
>
> The result: on open-ended "how does this work?" questions, **~10× fewer tokens**, with the answers no worse. A measured median, and a conservative one.
>
...
![What each tool hands back](/blog/response-shapes.svg)

### 2.4. Forced Calls

The second one bothered me more: the instructions push the tool onto the agent, hard.

codegraph spells it out: "use it instead of reading files," "call it before you Read," "don't grep or Read first," "almost any question → explore." codebase-memory-mcp pushes the same way in its tool descriptions ("use instead of grep") and auto-indexes on session start.

With codegraph the push lands, and calls fire even when the graph isn't the answer — a config, a small edit, a question it can't answer at all — and those wasted calls block the real work. (codebase-memory-mcp pushes too, but with its surface buried the agent ignored it, as we just saw.) Either way, that's exactly what frustrated me.
...
It is a rational thing to optimize, but it is the benchmark headline, not my workday.

And to their credit, none of this is hidden. codegraph's README says its token savings are scale-dependent and small on a normal codebase, and that the tool is overhead unless you query it directly. codebase-memory-mcp reports its biggest numbers on a handful of structural queries, not open-ended ones. They tell you, plainly, that the wins are measured on targeted scenarios.
