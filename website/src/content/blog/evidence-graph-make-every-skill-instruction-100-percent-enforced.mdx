...
> ## TL;DR
>
> - Write instructions in your skills and the agent [still will not follow them](https://arxiv.org/abs/2605.01771).
...
It means every single function under `src` has to answer every single rule.
...
You never write these comments yourself. The compile fails without them, so the agent writes them and hands them over. You only read the line.
...
The backend is a chain. The database schema cites the requirements, the API cites the requirements and the schema, and the tests cite that API. No table without a document behind it, and no API without a test on it.

### 3.3. Frontend

![Backend operations holding up hooks, screens, and journeys](https://ttsc.dev/evidence/frontend-light.svg)

The frontend starts from somebody else's document. The Swagger the backend publishes is the starting point, then hooks cite operations, screens cite hooks, and end-to-end journeys cite screens. This is where "the API is wired up but there is no screen yet" stops being a green build.

### 3.4. How Much It Changes

Spec Driven Development stops being a slogan and becomes something the build enforces. In our benchmark we built the same application twice with the same model, and the only difference was this plugin.
...
The bigger the application, the further plain coverage falls. With no way to know what is missing, you read everything again, fix what you find, and start over, until a round turns up nothing. That loop until dry ate 90% of the tokens in the left column. [The benchmark documentation](https://ttsc.dev/docs/benchmark/evidence) has the details.
...
- [Ten-minute setup guide](https://ttsc.dev/docs/setup/evidence)
