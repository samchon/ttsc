...
The guarded invariants are **`plugin runs == 1`**, **`stats/file` within the membership budget** (a missing resolution candidate is proven by notification rather than re-probed, samchon/ttsc#1261), and, for the graph scenario, **bounded probes per module** — the harness exits non-zero when either breaks.
