...
| --- | --- | --- |
| `TTSC_BENCH_WORK` | `./.work` (`performance.mjs`), `../../../ttsc-benchmark-work` (`graph.mjs`) | Clone working directory. The graph runner defaults outside the repo so a measured agent does not inherit ttsc's `CLAUDE.md` / `AGENTS.md` from a parent directory. |
