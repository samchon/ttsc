...
The question is tool-neutral and identical in both arms. The graph arm appends one neutral sentence — "This repository has a graph MCP tool available; make appropriate use of it when it fits the question." — because both harnesses now defer MCP tool schemas behind a tool-search step: without it the model shell-explores the repo and only then discovers the server, so the measurement prices a discovery accident rather than the tool. The sentence names no tool, forces no call, and tells the model only what an operator who installed the server already knows. All other guidance stays in the server's MCP instructions and tool descriptions, and the baseline arm gets the question unchanged.
