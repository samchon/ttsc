 ...
 * bytes alone, and the adapter marks it uncacheable where the host allows
 * that. Farm offers a JavaScript plugin no per-module opt-out of its
 * persistent cache, so after a type-only edit made while Farm was stopped, it
 * restored such a module and served its old output. Its `config` hook can turn
 * the cache off for the whole compile, measured on Farm 1.7.11: a module Farm
 * restores on the second compile of a browser target ran its transform again,
 * and no store was written. So Farm is asked here, before any delivery,
 * whether its records will exist: below its root (`hostToolDirectory`), or in
 * the fallback it accepts (`farmRecordFallback`), each proven by a write. Where
 * neither can be written the cache is turned off, which costs a full compile
 * per start, and the user is told once why, as a Node process warning, code
