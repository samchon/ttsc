...
// This pins the happy-path string-export, the object-export variant, invalid
// JSON, invalid shape, and BOM-prefixed files — the same edge cases the lint
// JSON loader guards against, applied to the banner domain.
//
// 1. Load a JSON file with a plain string value and one with a "text" object.
...
func TestJSONConfigLoader(t *testing.T) {
	...
	// String export.
	stringConfig := filepath.Join(root, "banner.config.json")
	writeFile(t, stringConfig, `"my banner text"`)
	raw, err := bannerLoadBannerJSONConfigFile(stringConfig)
	if err != nil {
		t.Fatal(err)
	}
	if raw != "my banner text" {
		t.Fatalf("JSON string config mismatch: %#v", raw)
	}

	// Object export.
	objectConfig := filepath.Join(root, "obj", "banner.config.json")
	...
	raw, err = bannerLoadBannerConfigFile(stringConfig)
	if err != nil {
		t.Fatal(err)
	}
	if raw != "my banner text" {
	...
	writeFile(t, bomConfig, "\xEF\xBB\xBF\"bom banner\"")
	...
	writeFile(t, filepath.Join(jsonRoot, "banner.config.json"), `"discovered json banner"`)
