...
/**
 * Path and extension helpers shared by the runtime resolver. These map between a
 * package's published JavaScript target and the raw TypeScript source it ships
 * in its place, and locate the package a source belongs to. They are pure
 * filesystem queries with no dependence on the emit model.
 */
...
export function typeScriptCounterpart(jsFile: string): string | null {
  for (const candidate of counterpartCandidates(jsFile)) {
    if (isFile(candidate)) {
      return candidate;
    }
  }
 ...
 * through a workspace symlink keys to the same on-disk identity regardless of
