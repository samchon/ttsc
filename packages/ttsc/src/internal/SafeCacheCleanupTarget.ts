 ...
 *
 * Produced by {@link resolveSafeCacheCleanupTargets} only after every
 * requested directory has been resolved and checked, so a caller that removes
 * `path` for each target never deletes a filesystem root or anything that
 * contains the project.
 */
export interface SafeCacheCleanupTarget {
  /**
   * The directory existed when the transaction inspected it. A target that did
   * not exist is still returned, so the caller can report it without treating
   * a missing cache as an error.
   */
  exists: boolean;
  ...
  /**
   * The directory as the caller requested it, lexically normalized. This is
   * the spelling to report back to the user.
   */
