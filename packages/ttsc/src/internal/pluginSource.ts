 ...
 * samchon/ttsc#1493): the state of a source directory, which of its
 * subdirectories the state passes over, and the ttsc and TypeScript-Go versions
 * every build is keyed on, which name output kept beyond the process that
 * produced it (samchon/ttsc#1483). This barrel is the one module path such a
 * consumer imports, so the implementation keeps one declaration per file.
 */
