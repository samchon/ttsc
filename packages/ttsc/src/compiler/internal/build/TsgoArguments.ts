 ...
 * actual output.
 */
export function createTsgoDiagnosticArgs(options: TtscCommonOptions): string[] {
  if (options.structuredDiagnostics !== true) return [];
  if (PassthroughFlags.forwardsInternalShadowFlag(options, "--pretty")) return [];
  return ["--pretty", "false"];
}
 ...
 */
export function createTsgoThreadingArgs(options: TtscCommonOptions): string[] {
  const args: string[] = [];
