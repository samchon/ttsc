...
export function nativePluginEnv(
  ...
  if (plugin?.stage === "transform") {
    ...
    if (linked.length !== 0) {
      env.TTSC_LINKED_PLUGINS_JSON = NativePluginArguments.serializeNativePlugins(linked);
    }
  }
  return env;
}
...
export function prepareBuildExecution(
  ...
  if (
    ...
    BuildTiming.recordTiming(timing, "ttsc plugin setup time", setupStartedAt);
  }
...
export function runProjectFreeTerminalFlag(
  ...
  if (!PassthroughFlags.forwardsProjectFreeTerminalTsgoFlag(options)) return null;
