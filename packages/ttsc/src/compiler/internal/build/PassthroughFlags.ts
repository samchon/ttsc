 ...
 * whether a flag turns the build into a terminal command, whether a flag is
 * one ttsc consumes internally. Every answer resolves the token through the
 * flag schema, so a question is decided by the flag's identity (any case, one
 * or two dashes, inline `=value`) exactly as the compiler would decide it.
 */
 ...
 */
export function forwardsTerminalTsgoFlag(options: TtscCommonOptions): boolean {
  return (
