 ...
 *
 * Two protocols coexist. The current one (v2) lives in `<lockDir>.v2`: its held
 * generation is `current/`, carrying a generation id and an owner, and it is
 * freed only by renaming `current/` onto the tombstone `retired/<generation>`,
 * so a stale releaser can never free a successor. The legacy protocol held
 * `<lockDir>` itself; a v2 process still recognizes and reclaims an abandoned
 * legacy lock, fenced by the legacy directory's mtime.
 */
