...
export const COMMONJS_PLUGIN_DESCRIPTOR_SHIM_SOURCE = [
  ...
  `    }`,
  `  }`,
  // One observation of a path, taken now and committed as an input later or
  // never: a resolution fingerprints its candidates before it runs, and keeps
  // only those it could have read once it settles.
  `  function observeInput(file) {`,
  ...
  `    return { afterSignature, beforeSignature, file, observed, observedRealpath };`,
  `  }`,
  `  function commitOneInput(observation) {`,
  `    const { afterSignature, beforeSignature, file, observed, observedRealpath } = observation;`,
  `    inputs.add(file);`,
  ...
  `    file = path.resolve(file);`,
  `    if (unstableInputHashes.has(file)) { inputs.add(file); return; }`,
  `    commitOneInput(observeInput(file));`,
  `  }`,
  `  function recordInput(file) {`,
  `    commitInput(observeInput(file));`,
  `  }`,
  `  function commitInput(observation) {`,
  `    const file = observation.file;`,
  `    commitOneInput(observation);`,
  ...
  `  function visitManifestTargets(value, directory, allowBare, visit, bases) {`,
  `    if (typeof value === "string") {`,
  `      if (value !== "" && (allowBare || value.startsWith("./") || value.startsWith("../"))) visitModuleCandidates(path.resolve(directory, value), visit, bases);`,
  `      return;`,
  `    }`,
  `    if (Array.isArray(value)) { for (const item of value) visitManifestTargets(item, directory, allowBare, visit, bases); return; }`,
  `    if (value && typeof value === "object") for (const item of Object.values(value)) visitManifestTargets(item, directory, allowBare, visit, bases);`,
  `  }`,
  `  function visitModuleCandidates(base, visit, bases) {`,
  `    const resolvedBase = path.resolve(base);`,
  `    if (bases.has(resolvedBase)) return;`,
  `    bases.add(resolvedBase);`,
  `    for (const candidate of moduleCandidates(resolvedBase)) visit(candidate);`,
  `    try {`,
  `      const manifest = JSON.parse(fs.readFileSync(path.join(resolvedBase, "package.json"), "utf8").replace(/^\uFEFF/, ""));`,
  `      visitManifestTargets(manifest.exports, resolvedBase, false, visit, bases);`,
  `      visitManifestTargets(manifest.module, resolvedBase, true, visit, bases);`,
  `      visitManifestTargets(manifest.main, resolvedBase, true, visit, bases);`,
  `    } catch {}`,
  `  }`,
  ...
  // each with the package directory of the search root it belongs to, or none
  // for a relative or absolute specifier. The search stops at the root whose
  // package `resolved` selects, so without one it covers every root.
  `  function visitResolutionCandidates(specifier, parent, resolved, visit, bases) {`,
  ...
  `          if (exact) local(base);`,
  `          if (!exact) visitModuleCandidates(base, local, bases);`,
  `        }`,
  `      } catch {}`,
  `      return;`,
  `    }`,
  ...
  `      visitModuleCandidates(packageDirectory, rooted, bases);`,
  `      if (subpath.length !== 0) visitModuleCandidates(path.join(packageDirectory, ...subpath), rooted, bases);`,
  `      if (candidateSelected(packageDirectory, selected)) break;`,
  `    }`,
  `  }`,
  `  function recordResolutionCandidates(specifier, parent, resolved) {`,
  `    visitResolutionCandidates(specifier, parent, resolved, recordInput, recordedModuleBases);`,
  `  }`,
  // A resolution fingerprints every candidate before it runs, so a
  // higher-priority one that appears while the descriptor evaluates cannot
  // bless the earlier result, and commits a fingerprint only once it settles:
  // a search stops at the first root whose package it selects, and the roots
  // after it were never read. A missing candidate there is proven absent by
  // the metadata of a directory as busy as a home directory, and would cost
  // the descriptor its cache proof for a path that cannot have steered it. A
  // resolution that fails read every root, and commits every fingerprint.
  `  function observeResolutionCandidates(specifier, parent) {`,
  `    const pending = [];`,
  `    const seen = new Set();`,
  `    visitResolutionCandidates(specifier, parent, undefined, (file, root) => {`,
  `      file = path.resolve(file);`,
  `      if (seen.has(file) || inputs.has(file)) return;`,
  `      seen.add(file);`,
  `      pending.push({ observation: observeInput(file), root });`,
  `    }, new Set());`,
  `    return pending;`,
  `  }`,
  `  function commitResolutionCandidates(pending, resolved) {`,
  `    let selected;`,
  `    try { const file = asFile(resolved); selected = file === undefined ? undefined : fs.realpathSync.native(file); } catch {}`,
  `    const roots = [...new Set(pending.flatMap(({ root }) => root === undefined ? [] : [root]))];`,
  `    const reached = roots.findIndex((root) => candidateSelected(root, selected));`,
  `    const read = new Set(reached === -1 ? roots : roots.slice(0, reached + 1));`,
  `    for (const { observation, root } of pending) if (root === undefined || read.has(root)) commitInput(observation);`,
  `  }`,
  ...
  `        const pending = observeResolutionCandidates(specifier, canonical);`,
  `        let resolved;`,
  `        try {`,
  `          resolved = globalThis.Bun.resolveSync(specifier, path.dirname(canonical));`,
  `          commitResolutionCandidates(pending, resolved);`,
  `          recordResolutionCandidates(specifier, canonical, resolved);`,
  `          recordFile(resolved);`,
  `          scan(resolved);`,
  `        } catch {`,
  `          if (resolved === undefined) commitResolutionCandidates(pending, undefined);`,
  ...
  `        const pending = observeResolutionCandidates(specifier, resolveContext.parentURL);`,
  `        let resolved;`,
  `        try {`,
  `          resolved = nextResolve(specifier, resolveContext);`,
  `        } catch (error) {`,
  `          commitResolutionCandidates(pending, undefined);`,
  ...
  `        commitResolutionCandidates(pending, url);`,
  ...
  `            const pending = observeResolutionCandidates(args.path, importer);`,
  `            let resolved;`,
  `            try {`,
