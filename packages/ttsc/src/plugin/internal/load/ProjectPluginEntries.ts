 ...
 *
 * Every entry carries the directory its specifier resolves from. A bare
 * package specifier resolves from the project root, never from the base config
 * an `extends` chain declared it in; a relative one resolves from the file that
 * wrote it.
 */
...
function discoverPackagePluginEntries(
  ...
  const projectPackageJson = PluginPackageResolution.findNearestPackageJson(project.root);
  if (projectPackageJson === undefined) {
    return [];
  }
  const projectPackageRoot = path.dirname(projectPackageJson);
  const projectManifest = PluginPackageResolution.readPackageManifest(projectPackageJson);
  ...
  for (const name of PluginPackageResolution.directDependencyNames(projectManifest)) {
    const packageJson = PluginPackageResolution.resolveDependencyPackageJson(name, projectPackageRoot);
    ...
    const baseDir = PluginPackageResolution.isRelativePluginSpecifier(transform)
      ? packageRoot
      : projectPackageRoot;
    const resolved = PluginPackageResolution.resolvePluginRequest(transform, baseDir);
...
function createConfiguredTransformSet(
  ...
  for (const entry of entries) {
    ...
    try {
      resolved.add(PluginPackageResolution.resolvePluginRequest(transform, entry.baseDir));
    } catch {
