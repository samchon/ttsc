...
**Use `expand_nodes` when `query_nodes` or `query_files` printed the right TypeScript declaration handle but not enough body.** It is deterministic: no fuzzy ranking, no file search, just the graph node(s) you named. For call-path, relation-flow, lifecycle, dispatch, or "how does X reach Y" questions, prefer `mode: "flow"` from the printed handle(s); it stays on downstream value-call/value-access path evidence. Use `mode: "source"` when you need a wider body window for one specific declaration; source expansion omits relationship edges because `query_nodes` already provided them.
