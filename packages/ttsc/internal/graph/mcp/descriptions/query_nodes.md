...
One broad fuzzy query (owner + action + nouns, e.g. "repository find manager query builder") returns the matched declarations with their calls, callers, types, blast radius, source, and exact handles: the whole cluster in one call, so you do not query one symbol at a time.
