...
//   - byte spans become 1-based line/col Evidence, with a short excerpt on edges;
...
func NewDump(g *Graph, project, tsconfig string, ignored map[string]bool, sources map[string]string) Dump {
	...
	for _, d := range g.Decorators {
		...
		for _, a := range d.Arguments {
			args = append(args, DumpDecoratorArgument{Text: a.Text, Literal: a.Literal})
		}
	...
	for _, n := range g.Nodes {
		...
		nodes = append(nodes, DumpNode{
			...
			Implementation: ctx.evidence(n.ImplementationFile, n.ImplementationPos, n.ImplementationEnd, false),
...
// span is absent or no source is available. includeText attaches a one-line
// excerpt (used for edge expressions, not declaration bodies).
func (c *dumpContext) evidence(file string, pos, end int, includeText bool) *DumpEvidence {
...
// excerpt of the expression text.
func (c *dumpContext) edgeEvidence(e *Edge) *DumpEvidence {
	...
	return c.evidence(file, e.Pos, e.End, true)
}
...
// excerpt is a single-line, length-capped slice of source text for inline
// evidence.
