...
//
// Splitting value-call from value-access is useful only if `service.run()` and
// `Tags.html\`x\`` do not also look like plain property reads. Otherwise MCP flow
// ranking cannot tell invoked behavior from state/accessor evidence.
//
