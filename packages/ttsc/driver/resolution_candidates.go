...
func ObserveProgramResolutions(prog *Program, cwd string) ProgramResolutionObservation {
  ...
  for first := 0; first < len(tasks); {
    ...
    for _, task := range group {
      ...
      if task.ResolvedFile != "" && prog.TSProgram.GetSourceFile(task.ResolvedFile) != nil {
        selected = append(selected, task.ResolvedFile)
      }
    }
    owner := group[0]
    if owner.Universal {
...
func appendResolutionPaths(
  ...
  for _, candidate := range observer.observedPaths() {
    if slicesContainResolutionPath(selected, candidate, caseSensitive) {
      continue
    }
    identityOnly := false
