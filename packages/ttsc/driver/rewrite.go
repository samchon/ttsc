...
// EmitAllRawWithHooks runs TypeScript-Go emit without ttsc output-text rewrites,
// while allowing ttsc-owned AST hooks immediately before JS/declaration print.
func (p *Program) EmitAllRawWithHooks(writeFile shimcompiler.WriteFile, hooks shimcompiler.EmitHooks) (*shimcompiler.EmitResult, []Diagnostic, error) {
  if p == nil || p.TSProgram == nil {
    return nil, nil, errors.New("driver: nil program")
  }
  wf := writeFile
  ...
  result := shimcompiler.EmitWithHooks(context.Background(), p.TSProgram, shimcompiler.EmitOptions{
    WriteFile: wf,
  }, hooks)
  if result == nil {
    return nil, nil, errors.New("driver: Emit returned nil")
