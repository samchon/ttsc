...
// cwd-resolution failure path.
func failGetwd() (string, error) {
  return "", errors.New("cwd boom")
}

// writeCommandProjectFile writes one project file below root, creating its
// directories.
func writeCommandProjectFile(t *testing.T, root, name, content string) {
  t.Helper()
