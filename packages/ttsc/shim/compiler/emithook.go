...
//
// `sourceFile` is the marking target, not the file that gets transformed. Along
// with two per-file constants (in-JS-file, JSX language variant) it is what
// upstream hands emitResolver.MarkLinkedReferencesRecursively, whose marks the
// import-elision transformer later reads back through EmitContext.ParseNode.
// Callers that run a plugin pass first must pass the PARSE tree here and the
// transformed tree to TransformSourceFile; upstream's emitter passes one file to
// both only because it has nothing in between. Re-check this when the pin moves:
// a new use of the parameter inside getScriptTransformers could make the two
// roles diverge further.
//
