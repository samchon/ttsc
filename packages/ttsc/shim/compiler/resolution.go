...
func ReplayProgramResolutions(tasks []ProgramResolutionTask, filesystem vfs.FS) bool {
	...
	first := tasks[0]
	host := resolutionHost{
		filesystem:       programResolutionFilesystem(first.program, filesystem),
...
// programResolutionFilesystem recreates the project-reference declaration
// view used by fileLoader. A referenced output declaration may be absent on
// disk while its source exists; the resolver must see the virtual declaration
// before Program substitutes that source into the loaded graph.
func programResolutionFilesystem(program *Program, filesystem vfs.FS) vfs.FS {
	if program == nil || filesystem == nil {
		return filesystem
	}
