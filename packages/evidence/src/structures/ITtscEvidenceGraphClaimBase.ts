...
export interface ITtscEvidenceGraphClaimBase<Type extends string> {
   ...
   * above it (`../../docs`), or on an absolute path, and it may be a symbolic
   * link or a Windows junction to a directory, which is read through. A
   * drive-relative Windows path such as `C:docs` is refused, because it
