...
/**
 * Security-focused TypeScript source rules from
 * `eslint-plugin-security`.
 *
 * Reports likely security smells — non-literal sinks for eval, file
 * I/O, regex construction, child-process spawning, cryptographic
 * primitives — that warrant human review even if no exploit is
 * statically provable.
 *
...
export interface ITtscLintSecurityRules {
  /**
   * Detect Trojan-Source bidi control characters (U+202A, U+202E,
   * ...) hidden inside source.
   *
  ...
  /**
   * Detect Buffer reads/writes called with `noAssert = true`, which
   * skips Node's offset/length bounds checks.
   *
   * The flag lets the offset slide past the buffer end and read
   * unrelated memory, so production code should never set it.
   *
  ...
  /**
   * Detect any import of `child_process` and any `exec`/`execSync`
   * call whose command argument is not a string literal.
   *
   * Non-literal commands are the canonical shell-injection sink in
   * Node services.
   *
  ...
  /**
   * Detect assignments setting `escapeMarkup = false` (or the
   * equivalent option on Handlebars/Mustache-style engines), which
   * turns off HTML entity escaping in template output.
   *
   ...
   * code-execution sink. The rule flags the call shape, not proven
   * taint.
   *
  ...
  /**
   * Detect `new Buffer(input)` constructions with non-literal
   * input — historical source of allocation-disclosure bugs.
   *
  ...
  /**
   * Detect `fs` calls (`readFile`, `writeFile`, `createReadStream`,
   * ...) whose filename argument is not a string literal.
   *
   * Dynamic filenames are the standard path-traversal sink; sanitise
   * or allow-list before the call.
   *
  ...
  /**
   * Detect `new RegExp(...)` construction whose pattern argument is
   * not a string literal.
   *
   * Caller-controlled patterns can both trigger catastrophic
   * backtracking and let an attacker reshape the matcher to bypass
   * intended validation.
   *
  ...
  /**
   * Detect `require(...)` calls whose specifier is computed at
   * runtime.
   *
   * A dynamic specifier lets caller-controlled data choose the module
   * to load, bypassing any module allow-list on Node.
   *
  ...
  /**
   * Detect dynamic bracket-access such as `obj[req.body.x] = ...`,
   * which can let caller-controlled keys overwrite prototype-shaped
   * properties or pull out unintended fields.
   *
  ...
  /**
   * Detect direct equality comparisons involving secret-like
   * identifiers (`if (token === expected)`) — use
  ...
  /**
   * Detect `crypto.pseudoRandomBytes`, which produces values that
   * are not cryptographically secure.
   *
   * Tokens, session ids, and key material must use
   * `crypto.randomBytes` (or Web Crypto's `getRandomValues`) instead.
   *
  ...
  /**
   * Detect regex literals with catastrophic backtracking potential
   * (ReDoS) — typically nested or overlapping quantifiers over the
   * same character set.
   *
   * Matching caller-controlled input against such a pattern can
