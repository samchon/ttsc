...
/**
 * Testing Library test source rules from
 * `eslint-plugin-testing-library`.
 *
 * Apply to TypeScript test sources that use any `@testing-library/*`
 * package and detect Testing Library anti-patterns — `container`
 * access, `ByTestId` overuse, missing `await` on async queries.
 *
...
export interface ITtscLintTestingLibraryRules {
   ...
   *
   * Without `await` the next assertion runs against the
   * pre-interaction DOM, which masks effects the user action was
   * meant to trigger.
   *
   ...
   *
   * They return a Promise that resolves once the element appears; an
   * unawaited query yields a pending Promise that no matcher can
   * assert against meaningfully.
   *
  ...
  /**
   * Require awaiting `waitFor`, `waitForElementToBeRemoved`, and
   * the other async Testing Library utilities.
   *
   * Skipping the `await` means the test moves on before the
   * predicate settles, so subsequent assertions race against the
   * wait.
   *
  ...
  /**
   * Validate JSX `data-testid` attribute values against a regex
   * pattern.
   *
   * The options object names the attribute and the pattern, keeping
   * test ids consistently formed across components.
   *
   ...
   * (`fireEvent.click(...)`).
   *
   * The helpers return `boolean` rather than a Promise, so the
   * `await` is a no-op that misleads readers into thinking the
   * helper is async.
   *
   ...
   * (`getBy*`, `queryBy*`).
   *
   * These queries return DOM nodes directly, so the `await` misleads
   * readers and can shadow a genuine missing `await` on an adjacent
   * `findBy*`.
   *
  ...
  /**
   * Reject `container` destructuring and DOM query methods on the
   * render result.
   *
   * `screen` queries the same document but matches how the user sees
   * the page, keeping tests accessibility-first and resilient to
   * layout refactors that move nodes inside the tree.
   *
  ...
  /**
   * Reject `debug`, `prettyDOM`, `logTestingPlaygroundURL`, and
   * related debugging utilities in committed tests.
   *
   * They print large DOM dumps to CI logs and only exist to help
  ...
  /**
   * Reject direct `@testing-library/dom` imports when a
   * framework-specific package is installed.
   *
   * Framework packages (`@testing-library/react`, ...) re-export the
   * same surface plus a `render` that wires the framework's
   * lifecycle — importing `dom` directly skips that wiring.
   *
  ...
  /**
   * Reject global RegExp flags (`/foo/g`) inside query text
   * matchers.
   *
   * The matcher reuses the regex across nodes, so a global regex's
   * persistent `lastIndex` state causes the second call to skip
   * matches the first one found.
   *
   ...
   *
   * Framework wrappers (`@testing-library/react`, ...) register
   * automatic cleanup, so explicit calls duplicate the unmount and
   * can race against the runner's between-test reset.
   *
   ...
   * (`.parentElement`, `.firstChild`, `.children`, ...).
   *
   * Traversal couples the test to incidental markup; another
   * semantic query names what the assertion actually cares about.
   *
   ...
   * since `fireEvent` is synchronous and the Promise is dropped.
   *
  ...
  /**
   * Reject `render(...)` inside lifecycle hooks (`beforeEach`,
   * etc.).
   *
   * Each test should render its component directly so the arrange
   * step is visible in-place and auto-cleanup runs between cases
   ...
   *
   * Test ids couple the test to incidental markup and skip the
   * accessibility tree that real users navigate; queries by role,
   * label, or text describe the UI in user-visible terms instead.
   *
  ...
  /**
   * Reject unnecessary `act(...)` wrappers around Testing Library
   * helpers.
   *
   * `render`, `fireEvent`, and `userEvent` already wrap their work
   * in `act`, so the extra wrapper is dead code that obscures the
   * real state change.
   *
  ...
  /**
   * Reject multiple assertions inside one `waitFor` callback —
   * split into separate `waitFor`s so each retry boundary is
   * narrow.
   *
  ...
  /**
   * Reject side effects inside `waitFor` callbacks — `waitFor`
   * retries the callback, so side effects fire repeatedly.
   *
   ...
   *
   * `waitFor` retries until the callback stops throwing, so the
   * captured snapshot is whichever pass happened to match — usually
   * an intermediate render rather than the settled UI the test
   * cares about.
   *
  ...
  /**
   * Require explicit assertions on the result of standalone
   * queries.
   *
   * A bare `screen.getByRole(...)` looks like an assertion but only
   * checks presence (and only for `getBy*`); adding `expect(...)`
   * makes the intent and the matched property obvious.
   *
   ...
   *
   * `findBy*` is the dedicated retry-aware query; the manual
   * combination duplicates its semantics and is easy to misconfigure
   * (wrong timeout, missing `await`).
   *
   ...
   *
   * `getBy*` already throws when nothing is found, so the extra
   * matcher only restates what the query promises and adds noise to
   * the failure trace.
   *
  ...
  /**
   * Match presence and absence assertions to the query variant
   * that already encodes the same semantic: `getBy*` for presence
   * (throws when missing), `queryBy*` for absence (returns `null`
   * when missing). Mixing the two yields confusing failure modes.
   *
   ...
   *
   * The helper retries the callback against the disappearing
   * element, but `getBy*` throws on the very state being waited for,
   * which produces a noisy error in the trace each retry.
   *
  ...
  /**
   * Prefer jest-dom document matchers (`toBeVisible`,
   * `toHaveTextContent`, ...) over generic equality checks on
   * Testing Library queries.
   *
   * The dedicated matchers explain failures in terms of the DOM
   * property they assert on, not a structural diff of nodes.
   *
   ...
   *
   * `screen` is the single global query target, so tests stay
   * consistent regardless of which component is mounted and the
   * render call's return value rarely needs destructuring.
   *
   ...
   *
   * `userEvent` simulates the full sequence of DOM events a real
   * user triggers (focus, keydown, input, change, ...) while
   * `fireEvent` dispatches a single event and skips intermediate
   * state.
   *
  ...
  /**
   * Prefer `userEvent.setup()` (the v14+ instance pattern) over
   * the static `userEvent.*` calls.
   *
   * The instance binds fresh pointer state per test, removing the
   * cross-test focus and click bookkeeping that the static API
   * leaks.
   *
  ...
  /**
   * Require the variable assigned from `render(...)` to use one of
   * the conventional names (`view`, `result`, ...).
   *
   * The name is a reading cue: a non-conventional one usually
   * signals that the destructured queries are being treated as a
   * component surface instead of a render artifact.
   *
