...
// wants to override `formatSemi` writes a sibling `rules` entry and
...
func TestFormatBlockRulesMapOverrideWins(t *testing.T) {
  ...
  if got := enabled["formatSemi"]; got != SeverityError {
    t.Errorf("formatSemi want error, got %v", got)
  }
  if got := enabled["formatQuotes"]; got != SeverityWarn {
