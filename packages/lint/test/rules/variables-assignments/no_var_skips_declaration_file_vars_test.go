...
// not trip the runtime no-var rule.
//
// TypeScript declaration files use `var` to describe globals and namespace
// exports. ESLint's no-var does not report those ambient declarations, so the
...
// 3. Run no-var and assert no finding is emitted.
func TestNoVarSkipsDeclarationFileVars(t *testing.T) {
  ...
  if len(findings) != 0 {
    t.Fatalf("no-var reported ambient declaration vars: %d findings", len(findings))
  }
}
