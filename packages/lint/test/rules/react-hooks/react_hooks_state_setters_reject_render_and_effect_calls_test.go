...
// TestReactHooksStateSettersRejectRenderAndEffectCalls verifies state setter placement rules.
//
...
// 2. Enable react-hooks/set-state-in-render and react-hooks/set-state-in-effect.
// 3. Assert each rule reports exactly its own call site.
func TestReactHooksStateSettersRejectRenderAndEffectCalls(t *testing.T) {
  source := `
function Widget() {
...
`
  ...
  findings := NewEngine(RuleConfig{
    "react-hooks/set-state-in-render": SeverityError,
    "react-hooks/set-state-in-effect": SeverityError,
