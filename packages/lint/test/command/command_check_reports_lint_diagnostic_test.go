...
//
// 1. Create a project with a noVar violation.
// 2. Run the check command with a discovered lint config enabling noVar.
...
func TestCommandCheckReportsLintDiagnostic(t *testing.T) {
  ...
  seedLintRules(t, root, map[string]string{"noVar": "error"})
  ...
  if code != 2 || stdout != "" || !strings.Contains(stderr, "[noVar]") {
