...
- [`typescript/await-thenable`](https://github.com/samchon/ttsc/blob/master/tests/test-lint/src/cases/await-thenable.ts): rejects `await` on a value that is neither a Promise nor a thenable, `for await...of` over a value that is not async iterable, and `await using` of a resource that is not async disposable (type-aware).
