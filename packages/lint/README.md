...
Contributor rules emit autofixes the same way built-ins do, call `ctx.ReportFix(node, message, edits...)` or `ctx.ReportRangeFix(pos, end, message, edits...)`. The `rule/astutil` package owns the byte-range helpers built-ins use (`NodeText`, `KeywordStart`, `FindKeyword`, `TokenRange`, `ForEachComment`, `ScanCommentGap`). `ForEachComment` visits every real comment in source order with its exact byte range; it scans only the gaps between the parser's own string, template, regex, and JSX tokens, so bytes that look like a comment inside a literal are never reported as one. See the [contributor autofix path](https://ttsc.dev/docs/development/walkthroughs/lint#the-contributor-autofix-path) section for the full contract and an example.
