...
- [`no-unused-expressions`](https://github.com/samchon/ttsc/blob/master/tests/test-lint/src/cases/no-unused-expressions.ts): rejects expression statements with no effect under ESLint's default semantics — directive prologues (arbitrary text, determined by AST position) and productive expressions such as `void promise()` are accepted, while tagged templates and misplaced strings are rejected; supports the upstream options.
