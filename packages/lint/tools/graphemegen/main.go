...
func main() {
  ...
  eastAsianWidthData := fetch("EastAsianWidth.txt")
  ...
  requireMarker("EastAsianWidth.txt", eastAsianWidthData, "EastAsianWidth-"+unicodeVersion+".txt")
  ...
  emojiPresentationRanges := parseBooleanEmojiProperty(emojiData, "Emoji_Presentation")
  defaultIgnorableRanges := parseBooleanDerivedProperty(derivedCoreData, "Default_Ignorable_Code_Point")
  eastAsianWideRanges := parseEastAsianWideProperties(eastAsianWidthData)
...
// parseBooleanEmojiProperty collects the code points carrying one boolean
// property of emoji-data.txt, whose lines are `code-point ; Property_Name`.
func parseBooleanEmojiProperty(content []byte, property string) []propertyRange {
  var ranges []propertyRange
  forEachDataLine(content, func(fields []string) {
    if len(fields) != 2 || fields[1] != property {
      return
    }
    lo, hi := parseCodePointRange(fields[0])
    ranges = append(ranges, propertyRange{lo: lo, hi: hi})
  })
  return normalizeRanges(property, ranges)
}

// parseBooleanDerivedProperty collects the code points carrying one boolean
// property of DerivedCoreProperties.txt. That file also holds two-value
// properties written as `code-point ; Name ; Value` (Indic_Conjunct_Break), so
// a boolean read must require exactly two fields rather than matching on the
// name alone.
func parseBooleanDerivedProperty(content []byte, property string) []propertyRange {
  var ranges []propertyRange
  forEachDataLine(content, func(fields []string) {
    if len(fields) != 2 || fields[1] != property {
      return
...
// parseEastAsianWideProperties collects the East_Asian_Width=W and =F code
// points, the two values `get-east-asian-width` charges two columns for.
//
// Only code points the file lists are collected. UAX #11 assigns a default of
// W to unassigned code points in several CJK blocks through `@missing` lines,
// but the JavaScript package the width contract names derives its table from
// the listed entries alone, so an unassigned code point in one of those blocks
// (U+2E9A, U+A4C7, U+FF00) is narrow there. Applying the `@missing` defaults
// here would measure those three wide and diverge from the oracle.
func parseEastAsianWideProperties(content []byte) []propertyRange {
  var ranges []propertyRange
  forEachDataLine(content, func(fields []string) {
    if len(fields) < 2 {
      panic(fmt.Sprintf("unexpected EastAsianWidth fields: %q", fields))
    }
  ...
  return normalizeRanges("East_Asian_Width", ranges)
}
...
func renderTables(
  ...
  out.WriteString(".\n")
  out.WriteString("// Sources: GraphemeBreakProperty.txt, DerivedCoreProperties.txt, emoji-data.txt,\n")
  out.WriteString("// and EastAsianWidth.txt.\n")
...
`)
  ...
  renderPropertyRanges(&out, "emojiPresentationRanges", "unicodeRange", emojiPresentation, false)
  renderPropertyRanges(&out, "defaultIgnorableRanges", "unicodeRange", defaultIgnorable, false)
  renderPropertyRanges(&out, "eastAsianWideRanges", "unicodeRange", eastAsianWide, false)
