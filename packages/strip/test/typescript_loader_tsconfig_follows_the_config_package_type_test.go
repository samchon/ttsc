...
func TestTypeScriptLoaderTsconfigFollowsTheConfigPackageType(t *testing.T) {
  ...
  for _, testCase := range []struct {
    ...
    {filepath.Join(root, "cjs", "strip.config.ts"), "CommonJS", `a manifest with no "type" means CommonJS`},
    {filepath.Join(root, "esm", "strip.config.ts"), "ESNext", `an explicit "type": "module"`},
    {filepath.Join(root, "esm", "nested", "strip.config.ts"), "CommonJS", "the nearest manifest outranks a module-typed ancestor"},
    {filepath.Join(root, "cjs", "declared-module", "strip.config.ts"), "ESNext", "the nearest manifest outranks a CommonJS ancestor"},
    {filepath.Join(root, "esm", "unparseable", "strip.config.ts"), "CommonJS", "a manifest that does not parse still bounds the scope"},
    {filepath.Join(deep, "strip.config.ts"), "ESNext", "a directory with no manifest defers to the enclosing package"},
    {filepath.Join(root, "esm", "strip.config.cts"), "ESNext", ".cts is left to its extension"},
    {filepath.Join(root, "cjs", "strip.config.mts"), "ESNext", ".mts is left to its extension"},
    {filepath.Join(root, "cjs", "strip.config.js"), "CommonJS", "an ambiguous .js follows the same scope"},
  } {
    dir := t.TempDir()
