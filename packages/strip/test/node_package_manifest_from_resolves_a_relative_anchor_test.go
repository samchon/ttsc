...
func TestNodePackageManifestFromResolvesARelativeAnchor(t *testing.T) {
  ...
  writeFile(t, filepath.Join(nested, "strip.config.ts"), "export default {};\n")
  t.Chdir(nested)
  ...
  if !filepath.IsAbs(got) {
    t.Fatalf("stripNodePackageManifestFrom returned the relative path %q; a child process resolving it against its own cwd would miss", got)
  }
