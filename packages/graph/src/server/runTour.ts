...
/**
 * Whether a candidate seed would only say again what a chosen seed already
 * says: the same file, and a name the chosen one contains word for word.
 * `renderNewElementScene` and `renderNewElementSceneThrottled` are one function
 * and its throttle, and they took two of the five seeds on Excalidraw's edit
 * pipeline while the mutation and history layers took none.
 */
 ...
 * of speech: the asker writes "scene mutation" and "persistence", the symbol is
 * `mutateElement` and `persist`. Inflection alone (`-ing`, `-ed`, `-s`) does
 * not bridge that — "mutation" and "mutate" share only five characters, one
 * short of the prefix rule below — so a tour of Excalidraw's edit pipeline
 * seeded four renderers and never surfaced the mutation, history, or
 * persistence layers the question named, and the model had to go find them
 * itself in twenty-one further calls.
 *
 * Stripping the derivational suffixes as well collapses both spellings onto the
 * same stem, so the noun in the question finds the verb in the code.
 */
