...
On codegraph's own agent-cost benchmark, an agent answers an architecture question from the graph alone, reading zero files, for 74% fewer tokens and 88% fewer tool calls than the same agent without it, averaged across two repositories. See the [benchmark](https://ttsc.dev/docs/graph/benchmark) for the numbers and method.
