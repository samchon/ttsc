 ...
 * publishes raw TypeScript cannot load. ttsx compiles it with tsgo instead — a
 * real build that also lowers TypeScript-only constructs like `enum` — and the
 * resolve hook serves the emitted `.js` (a module, from the package `type`).
 *
