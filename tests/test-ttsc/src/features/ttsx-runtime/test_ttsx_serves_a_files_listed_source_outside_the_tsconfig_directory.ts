 ...
 * synthetic tsconfig with `rootDir: "/"` and `files: [loader, config]`, so the
 * config — anywhere on disk — is part of the build. The runtime hooks must
 * serve such a file from the build's emit (here as an ES module, `module:
 * ESNext`), not type-strip it. The bound is the project's `rootDir`; a root
 * `rootDir` (`/`) must still include everything (a naive separator append
 * yields `//`, which matches nothing).
 *
 * 1. Create a loader and a config in different directories, with a tsconfig whose
 *    `rootDir` is `/` and whose `files` lists both by absolute path.
