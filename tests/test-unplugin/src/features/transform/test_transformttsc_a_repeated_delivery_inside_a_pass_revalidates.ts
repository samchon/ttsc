...
/**
 * Verifies a module delivered twice inside one pass revalidates the second
 * time.
 *
 * The constant-time shortcut is a statement about a module's _first_ delivery
 * in a pass. A bundler asking for the same module again is the one signal the
 * pass itself provides that something may have moved, so the retained
 * generation must not silently answer it from the pass gate.
 *
