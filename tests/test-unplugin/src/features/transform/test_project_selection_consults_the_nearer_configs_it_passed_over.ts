 ...
 *
 * The walk that finds a file's nearest config probed every nearer directory
 * and found no config there, then dropped those observations. A
 * `tsconfig.json` created beside the file re-routes it to another project, yet
 * no registered input named that path, so a watching host kept the module it
 * had transformed under the old project (samchon/ttsc#1543).
 *
 * 1. Select a file two directories below the only config.
 * 2. Assert both nearer candidates are consulted, as missing watch inputs, and
 *    the selected config is not among them.
 * 3. Create the nearer config and select again: it is selected, only the one
 *    candidate still nearer is consulted, and the old evidence no longer holds.
 */
export async function test_project_selection_consults_the_nearer_configs_it_passed_over(): Promise<void> {
  const root = fs.realpathSync.native(TestProject.tmpdir("ttsc-nearer-config-"));
  ...
  assert.deepEqual(
    inputs.map((input: { evidence: { missing: boolean } }) => input.evidence.missing),
    [true, true],
  );
