...
function writeGoPlugin(dir: string): void {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "  }",
      "",
      '  if delay := numberValue(cfg, "transformDelayMs"); delay > 0 {',
      ...
      "    raced := false",
      '    if raceFile != "" && samePath(file, raceFile) && raceMarker != "" {',
      ...
      '    if raced && raceFile != "" && samePath(file, raceFile) && stringValue(cfg, "snapshotRaceOriginal") != "" {',
