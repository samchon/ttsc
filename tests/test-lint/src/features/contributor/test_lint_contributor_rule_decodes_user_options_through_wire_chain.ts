...
/**
 * Verifies lint contributor protocol: a user-supplied `markers` array
 * threads from `lint.config.ts` through the wire chain into the
 * contributor's Go `ctx.DecodeOptions` call.
 *
 * The unit test for the same surface
 * (`contributor_rule_decodes_options_through_public_context_test.go`)
 * constructs an in-process synthetic rule and bypasses the build
 * pipeline. This scenario crosses every real boundary: JS factory
 * resolves the demo package, ttsc copies its Go source into the host
 * binary, the synthesized `init()` registers `demo/no-marker-comment`
 * before `main`, the engine populates `ctx.Options` with the user's
 * blob, and the rule's `DecodeOptions` reads `markers`. A regression
 * in any boundary would leave the rule silently running with default
 * markers, so the assertion checks the *negative* anchor (a TODO
 ...
 * 3. Assert exactly one diagnostic fires — the XXX line — and the TODO
 *    line is left untouched because the user-supplied options narrowed
 *    the marker set.
 */
