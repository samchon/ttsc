 ...
 *    source.
 * 2. Symlink the workspace `@ttsc/lint` and copy the `lint-contributor-demo`
 *    fixture into the temp project's `node_modules` so the JS factory's
 *    `require.resolve` reaches it.
...
export const test_lint_contributor_plugin_rule_fires_through_single_diagnostic_stream =
  () => {
    ...
    try {
      ...
      TestProject.copyDirectory(demoSource, demoTarget);
