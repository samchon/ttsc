 ...
 *
 * The fixture pins the statement-split + indent cascade through a
 * function -> if -> for -> bare-block stack where statements are crammed onto
 * shared lines and mis-indented at several depths at once. Each statement must
 * land on its own line at exactly `depth * 2` columns while the pre-aligned
 * closing braces (which are not statements and so are never touched by the
 * indent rule) stay put — proving the depth counter tracks every enclosing
 * Block independently and the two rules compose without fighting.
 *
  ...
  try {
    ...
    const result = TestProject.spawn(
      ...
      ["format", "--cwd", root],
      {
        ...
        env: {
          PATH: goPath(),
          TTSC_CACHE_DIR: TestProject.tmpdir("ttsc-lint-format-nested-cache-"),
