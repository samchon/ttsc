 ...
 * preserved: the failure-surfacing logic only runs for a *configured* plugin.
 * A disabled plugin is never invoked, so a would-be failing envelope can never
 ...
 *
 * 1. `typiaPlugin: false` → compile never calls the plugin and builds the
 *    source directly to a success.
