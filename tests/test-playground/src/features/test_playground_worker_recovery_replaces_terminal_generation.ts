 ...
 *
 * 1. Recover a plain terminal result and assert invalidate-reset-fail ordering.
 * 2. Recover a message-only tgrid error carrying the terminal marker.
 * 3. Reject a transient compile error without invoking any recovery action.
 */
export const test_playground_worker_recovery_replaces_terminal_generation =
  async (): Promise<void> => {
    ...
    assert.deepEqual(order, ["invalidate", "reset", "fail"]);
