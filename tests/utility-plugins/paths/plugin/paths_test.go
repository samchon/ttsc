// Unit tests for the paths output transform.
//
// These tests pin the runtime-rewrite contract without loading a full tsconfig:
// exact path patterns outrank wildcards, JS require/import and declaration
// import types are rewritten, and already-runtime-addressable specifiers stay
// unchanged.
package main
...
func testResolver() *pathsResolver {
  // The resolver mirrors a project with src -> dist emit and one exact alias
  // plus one wildcard alias. Missing targets appear first to prove fallback to
  // later paths entries works.
