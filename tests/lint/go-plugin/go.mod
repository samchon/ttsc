module github.com/samchon/ttsc/tests/lint/go-plugin

go 1.26

// Engine + config tests for the lint package live here as a separate
// Go module, mirroring the workspace's `tests/go-transformer/` layout.
// The rule corpus is exercised end-to-end via `tests/lint/cases.test.cjs`
// — these Go tests cover the plumbing the e2e runner can't see (engine
// dispatch, severity parsing, registry).
//
// Go workspace mode in 1.26 doesn't fully short-circuit proxy resolution
// for placeholder-versioned multi-module repos like the typescript-go
// shim graph, so we add `replace` directives that anchor every shim
// plus the lint package itself at the in-tree checkout. At CI time,
// scripts/test-go-lint.cjs rewrites these to absolute paths via the
// materialized go.work; in development mode the relative paths below
// resolve from this go.mod.
...
replace (
	...
	github.com/samchon/ttsc/packages/lint/go-plugin => ../../../packages/lint/go-plugin
)
