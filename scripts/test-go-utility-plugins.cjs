...
const packages = [
  { name: "banner", tests: "test-banner" },
  { name: "paths", tests: "test-paths" },
  { name: "strip", tests: "test-strip" },
];

for (const { name, tests: testPackage } of packages) {
  const source = path.join(root, "packages", name);
  const tests = path.join(root, "tests", testPackage);
